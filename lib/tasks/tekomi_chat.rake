...
namespace :tekomi do
  desc 'Start interactive chat with Tekomi assistant - Usage: rake tekomi:chat[assistant_id] or rake tekomi:chat -- assistant_id'
  task :chat, [:assistant_id] => :environment do |_, args|
    assistant_id = args[:assistant_id] || ARGV[1]

    unless assistant_id
      puts '❌ Please provide an assistant ID'
      puts 'Usage: rake tekomi:chat[assistant_id]'
      puts "\nAvailable assistants:"
      Tekomi::Assistant.includes(:account).each do |assistant|
        puts "  ID: #{assistant.id} - #{assistant.name} (Account: #{assistant.account.name})"
      end
      exit 1
    end

    assistant = Tekomi::Assistant.find_by(id: assistant_id)
    unless assistant
    ...
    chat_session = TekomiChatSession.new(assistant)
    chat_session.start
  end
end

class TekomiChatSession
  ...
  def generate_assistant_response
    runner = Tekomi::Assistant::AgentRunnerService.new(assistant: @assistant, callbacks: build_callbacks)
