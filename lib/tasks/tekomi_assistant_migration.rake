...
namespace :tekomi do
  namespace :assistant_migration do
    desc 'Generate structured migration drafts. Usage: rake tekomi:assistant_migration:generate IDS=1,2,3 LIMIT=50 ' \
         'OUTPUT=tmp/tekomi_migration.jsonl'
    task generate: :environment do
      assistants = TekomiAssistantMigrationTask.assistants
      output_path = ENV.fetch('OUTPUT', Rails.root.join('tmp/tekomi_assistant_migration_drafts.jsonl').to_s)

      FileUtils.mkdir_p(File.dirname(output_path))
      processed = 0

      File.open(output_path, 'w') do |file|
        TekomiAssistantMigrationTask.each_assistant(assistants) do |assistant|
          result = Tekomi::AssistantMigration::InstructionClassifier.new(assistant: assistant).perform
          file.puts(JSON.generate(result))
          processed += 1
          puts "Generated migration draft for assistant #{assistant.id} (#{processed}/#{TekomiAssistantMigrationTask.assistant_count(assistants)})"
        end
      end
    ...
    desc 'Apply reviewed migration drafts. Usage: rake tekomi:assistant_migration:apply INPUT=tmp/reviewed.jsonl DRY_RUN=true'
    task apply: :environment do
      input_path = ENV.fetch('INPUT')
      dry_run = TekomiAssistantMigrationTask.truthy?('DRY_RUN', default: true)

      results = TekomiAssistantMigrationTask.apply_drafts(
    ...
    desc 'Restore conversation message config from migration backup. Usage: rake tekomi:assistant_migration:restore_messages IDS=1,2 DRY_RUN=true'
    task restore_messages: :environment do
      dry_run = TekomiAssistantMigrationTask.truthy?('DRY_RUN', default: true)
      results = TekomiAssistantMigrationTask.restore_conversation_messages(dry_run: dry_run)
...
class TekomiAssistantMigrationTask
  ...
  class << self
    def assistants
      ...
      scope = Tekomi::Assistant.includes(:account, :tekomi_inboxes, :scenarios)
    ...
    def restore_conversation_messages(dry_run:)
      ENV.fetch('IDS').split(',').filter_map { |id| id.strip.presence }.map do |assistant_id|
        assistant = Tekomi::Assistant.find(assistant_id)
    ...
    def apply_draft(payload, line_number:, dry_run:)
      ...
      Tekomi::AssistantMigration::DraftApplier.new(
        assistant: assistant,
        draft: draft,
        dry_run: dry_run
    ...
    def restore_conversation_messages_for(assistant, dry_run:)
      original_config = assistant.config.dig(
        Tekomi::AssistantMigration::DraftApplier::CONFIG_KEY,
        Tekomi::AssistantMigration::DraftApplier::ORIGINAL_VALUES_KEY,
        'config'
      )
    ...
    def migration_eligible_scope(scope)
      scope.left_outer_joins(:scenarios)
           .joins(:tekomi_inboxes)
           .where("NULLIF(tekomi_assistants.config->>'instructions', '') IS NOT NULL")
           .where("tekomi_assistants.response_guidelines IS NULL OR tekomi_assistants.response_guidelines = '[]'::jsonb")
           .where("tekomi_assistants.guardrails IS NULL OR tekomi_assistants.guardrails = '[]'::jsonb")
           .where(tekomi_scenarios: { id: nil })
           .distinct
    end

    def migration_candidate?(assistant)
      assistant.config['instructions'].present? &&
        assistant.tekomi_inboxes.size.positive? &&
    ...
    def csv_assistant(row)
      ...
      CsvAssistant.new(
        ...
        tekomi_inboxes: CsvAssociation.new(inbox_count: normalize_integer(row['inbox_count'])),
        scenarios: []
      )
    end
