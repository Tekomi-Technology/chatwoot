# Generates the LLM welcome summary for the Tekomi Overview page from the
# assistant's stats hash (see Tekomi::AssistantStatsBuilder). Renders the
# tekomi_overview_summary.liquid prompt and returns markdown.
class Tekomi::OverviewSummaryService < Tekomi::BaseTaskService
  ...
  def system_prompt
    Liquid::Template.parse(prompt_from_file('tekomi_overview_summary')).render(prompt_variables)
  end
  ...
  # response, so it should not consume or be blocked by the tekomi_responses quota.
