...
RSpec.describe Tekomi::BaseTaskService do
  ...
  before do
    InstallationConfig.where(name: 'TEKOMI_OPEN_AI_API_KEY').destroy_all
    create(:installation_config, name: 'TEKOMI_OPEN_AI_API_KEY', value: 'test-key')
    # Stub tekomi enabled check to allow OSS specs to test base functionality
  ...
  describe '#conversation_messages' do
    ...
    it 'respects token limit' do
      ...
      expect(total_length).to be <= Tekomi::BaseTaskService::TOKEN_LIMIT
    end

    it 'respects start_from offset for token counting' do
      # With a start_from offset, fewer messages should fit
      start_from = Tekomi::BaseTaskService::TOKEN_LIMIT - 100
  ...
  describe '#make_api_call' do
    ...
    context 'when tekomi_tasks is disabled' do
      ...
      it 'returns disabled error' do
        ...
        expect(result[:error]).to eq(I18n.t('tekomi.disabled'))
    ...
    context 'when API key is not configured' do
      before do
        InstallationConfig.find_by(name: 'TEKOMI_OPEN_AI_API_KEY')&.destroy
      ...
      it 'returns api key missing error' do
        ...
        expect(result[:error]).to eq(I18n.t('tekomi.api_key_missing'))
    ...
    it 'prefers account overrides over supplied feature fallback models' do
      account.update!(tekomi_models: { 'help_center_article_generation' => 'gpt-4.1' })
  ...
  describe '#api_key' do
    ...
    it 'uses account OpenAI hook for editor task services' do
      ...
      editor_services = [
        Tekomi::RewriteService.new(account: account, content: 'Text', operation: 'improve', conversation_display_id: conversation.display_id),
        Tekomi::SummaryService.new(account: account, conversation_display_id: conversation.display_id),
        Tekomi::ReplySuggestionService.new(account: account, conversation_display_id: conversation.display_id, user: user),
        Tekomi::LabelSuggestionService.new(account: account, conversation_display_id: conversation.display_id),
        Tekomi::FollowUpService.new(
          account: account,
