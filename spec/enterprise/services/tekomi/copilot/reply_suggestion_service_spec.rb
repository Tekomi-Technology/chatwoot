...
RSpec.describe Tekomi::Copilot::ReplySuggestionService do
  subject(:service) do
    described_class.new(
      assistant: assistant,
  ...
  let(:runner) do
    instance_double(
      Tekomi::Assistant::AgentRunnerService,
  ...
  it 'generates the suggestion with the Assistant Agent Runner in reply suggestion mode' do
    ...
    expect(Tekomi::Assistant::AgentRunnerService).to have_received(:new).with(
      assistant: assistant,
      conversation: conversation,
      source: Tekomi::Assistant::AgentRunnerService::REPLY_SUGGESTION_SOURCE
    )
  ...
  it 'discards the action without charging when an outgoing reply already exists' do
    ...
    end.not_to(change { account.reload.custom_attributes['tekomi_responses_usage'].to_i })

    expect(Tekomi::Assistant::AgentRunnerService).not_to have_received(:new)
  ...
  it 'increments response usage after persisting the suggestion' do
    expect do
      service.generate_response
    end.to change { account.reload.custom_attributes['tekomi_responses_usage'].to_i }.by(1)
  end
