...
RSpec.describe Tekomi::Copilot::ChatService do
  ...
  let!(:copilot_message) do
    create(
      :tekomi_copilot_message, account: account, copilot_thread: copilot_thread
    )
  end
  ...
  before do
    ...
    InstallationConfig.find_or_create_by(name: 'TEKOMI_OPEN_AI_API_KEY') do |c|
      c.value = 'test-key'
    end
  ...
  describe '#generate_response' do
    ...
    it 'uses the copilot feature model' do
      account.update!(tekomi_models: { 'copilot' => 'gpt-5.2' })
