...
RSpec.describe Tekomi::Assistant::AgentRunnerService do
  let(:account) { create(:account) }
  let(:inbox) { create(:inbox, account: account) }
  let(:contact) { create(:contact, account: account) }
  let(:conversation) { create(:conversation, account: account, inbox: inbox, contact: contact) }
  let(:assistant) { create(:tekomi_assistant, account: account) }
  let(:scenario) { create(:tekomi_scenario, assistant: assistant, enabled: true) }
  ...
  before do
    ...
    scenarios_relation = instance_double(Tekomi::Scenario)
  ...
  describe '#build_and_wire_agents' do
    it 'keeps only reply-safe tools and excludes scenarios in reply suggestion mode' do
      faq_tool = Tekomi::Tools::FaqLookupTool.new(assistant)
      handoff_tool = Tekomi::Tools::HandoffTool.new(assistant)
      get_tool = Tekomi::Tools::HttpTool.new(assistant, create(:tekomi_custom_tool, account: account, http_method: 'GET'))
      post_tool = Tekomi::Tools::HttpTool.new(assistant, create(:tekomi_custom_tool, account: account, http_method: 'POST'))
      private_note_tool = Tekomi::Tools::AddPrivateNoteTool.new(assistant)
  ...
  describe '#generate_response' do
    ...
    context 'when the latest user message is multimodal' do
      ...
      it 'stores multimodal trace payloads in runner context' do
        expect(mock_runner).to receive(:run) do |_input, context:, max_turns:|
          expect(context[:tekomi_v2_trace_input]).to include('image_url')
          expect(context[:tekomi_v2_trace_current_input]).to include('image_url')
          expect(max_turns).to eq(10)
        end
    ...
    context 'when handoff tool was called during agent execution' do
      let(:runner_context) { { tekomi_v2_handoff_tool_called: true } }
      let(:mock_result) do
        instance_double(
          Agents::RunResult,
          output: {
    ...
    context 'when structured response parts contain invalid values' do
      let(:assistant) { create(:tekomi_assistant, account: account, config: { 'feature_citation' => true }) }
      let(:mock_result) do
        instance_double(
          Agents::RunResult,
          output: {
    ...
    context 'when a structured response exceeds the channel limit' do
      let(:channel_limit_rewrite) do
        ...
        runner_context = {
          session_id: "#{account.id}_#{conversation.display_id}",
          state: { Tekomi::Assistant::CITATION_SOURCES_STATE_KEY => { 1 => 101, 2 => 202 } },
      ...
      before do
        ...
        allow(Tekomi::MessageLengthLimit).to receive(:for).with(conversation).and_return(140)
      ...
      it 'reserves space for rendered citations and restores their original indexes' do
        original_response_parts = Tekomi::Assistant::ResponseParts.from_response(mock_result.output)
        ...
        expect(
          Tekomi::Assistant::ResponseParts.from_response(result).customer_message_content(
      ...
      it 'rejects a rewrite that changes the response part citation order' do
        ...
        expect(result['reasoning']).to eq('Error occurred: Tekomi response rewrite changed the response part citation order')
      end
    end
    ...
    context 'when an error occurs' do
      ...
      it 'logs error details' do
        expect(Rails.logger).to receive(:error).with('[Tekomi V2] AgentRunnerService error: Test error')
        ...
        before do
          allow(mock_runner).to receive(:run) do
            service.send(:track_handoff_usage,
                         Tekomi::Tools::HandoffTool.new(assistant).name,
                         Tekomi::Tools::HandoffTool.new(assistant).name,
  ...
  describe '#dynamic_trace_attributes' do
    ...
    it 'adds serialized trace input attributes when present in context' do
      context = {
        ...
        tekomi_v2_trace_input: '[{"role":"user","content":[{"type":"image_url","image_url":{"url":"https://example.com/image.jpg"}}]}]'
      }
  ...
  describe 'InstrumentationAttributeProvider' do
    subject(:provider) { Tekomi::Assistant::InstrumentationAttributeProvider.new(service) }
    ...
    it 'marks a protected generation as not discarded when no newer message has arrived' do
      ...
      attribute_provider = Tekomi::Assistant::InstrumentationAttributeProvider.new(runner_service)
      message = instance_double(RubyLLM::Message, tool_calls: {})
  ...
  describe '#add_usage_metadata_callback' do
    it 'sets credit_used=false when handoff tool is used' do
      ...
      tool_complete_callback.call(Tekomi::Tools::HandoffTool.new(assistant).name, 'ok', context_wrapper)
    ...
    it 'registers handoff tracking callback when OTEL is disabled' do
      ...
      context_wrapper = Struct.new(:context).new({ state: { tekomi_v2_handoff_tool_completed: true } })

      expect(tool_complete_callback).not_to be_nil
      tool_complete_callback.call(Tekomi::Tools::HandoffTool.new(assistant).name, 'ok', context_wrapper)

      expect(context_wrapper.context[:tekomi_v2_handoff_tool_called]).to be true
