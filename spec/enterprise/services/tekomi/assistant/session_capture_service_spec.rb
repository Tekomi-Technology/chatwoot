...
RSpec.describe Tekomi::Assistant::SessionCaptureService do
  ...
  describe '#capture' do
    it 'creates the session' do
      expect { service.capture }.to change(Tekomi::AgentSession, :count).by(1)
    end
    ...
    it 'does nothing when there is no run result' do
      ...
      expect { service.capture }.not_to change(Tekomi::AgentSession, :count)
    end
  ...
  describe '#capture!' do
    ...
    it 'stores only selected customer-visible document citations' do
      cited_document = create(:tekomi_document, assistant: assistant, external_link: 'https://help.example.com/reset-password')
      cited_faq = create(:tekomi_assistant_response, assistant: assistant, documentable: cited_document)
      retrieved_document = create(:tekomi_document, assistant: assistant, external_link: 'https://help.example.com/change-email')
      retrieved_faq = create(:tekomi_assistant_response, assistant: assistant, documentable: retrieved_document)
      user = create(:user, account: account)
      used_faq = create(:tekomi_assistant_response, assistant: assistant, documentable: user, status: :approved)
      ...
      run_context[:state] = {
        ...
        Tekomi::Assistant::CITATION_SOURCES_STATE_KEY => { 1 => cited_document.id, 2 => retrieved_document.id }
      }
      run_result.output = {
      ...
      result_message.update!(
        additional_attributes: {
          Tekomi::Assistant::ResponseParts::MESSAGE_ATTRIBUTE_KEY => run_result.output['response_parts']
        }
      )
    ...
    it 'does not store citations when citations are disabled' do
      document = create(:tekomi_document, assistant: assistant, external_link: 'https://help.example.com/reset-password')
      run_context[:state][Tekomi::Assistant::CITATION_SOURCES_STATE_KEY] = { 1 => document.id }
      run_result.output = {
        'response_parts' => [{ 'text' => 'Reset your password from settings.', 'citation_indexes' => [1] }],
        'reasoning' => 'Used the password FAQ'
      }
    ...
    it 'extracts every scenario that authored a message in the current turn' do
      first_scenario = create(:tekomi_scenario, assistant: assistant, account: account)
      second_scenario = create(:tekomi_scenario, assistant: assistant, account: account)
      run_context[:conversation_history] = [
    ...
    it 'does not capture a scenario belonging to another assistant' do
      scenario = create(:tekomi_scenario, assistant: create(:tekomi_assistant, account: account), account: account)
      run_context[:conversation_history] = [
