...
RSpec.describe Tekomi::Onboarding::WebsiteAnalyzerService do
  let(:website_url) { 'https://example.com' }
  let(:service) { described_class.new(website_url) }
  let(:mock_crawler) { instance_double(Tekomi::Tools::SimplePageCrawlService) }
  ...
  before do
    ...
    allow(Tekomi::Tools::SimplePageCrawlService).to receive(:new).and_return(mock_crawler)
