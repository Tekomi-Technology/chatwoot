...
RSpec.describe Tekomi::Tools::SearchDocumentationService do
  ...
  describe '#execute' do
    ...
    let(:documentable) { create(:tekomi_document, external_link: external_link) }

    context 'when matching responses exist' do
      before do
        response.update(documentable: documentable)
        allow(Tekomi::AssistantResponse).to receive(:search).with(question).and_return([response])
      end
    ...
    context 'when no matching responses exist' do
      before do
        allow(Tekomi::AssistantResponse).to receive(:search).with(question).and_return([])
      end
