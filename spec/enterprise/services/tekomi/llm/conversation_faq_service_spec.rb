...
RSpec.describe Tekomi::Llm::ConversationFaqService do
  let(:tekomi_assistant) { create(:tekomi_assistant) }
  let(:conversation) { create(:conversation, account: tekomi_assistant.account, first_reply_created_at: Time.zone.now) }
  let(:service) { described_class.new(tekomi_assistant, conversation) }
  let(:embedding_service) { instance_double(Tekomi::Llm::EmbeddingService) }
  ...
  before do
    ...
    allow(Tekomi::Llm::EmbeddingService).to receive(:new).and_return(embedding_service)
  ...
  describe '#generate_suggestions' do
    context 'when successful' do
      ...
      it 'uses the conversation FAQ generation feature model' do
        ...
        described_class.new(tekomi_assistant, conversation).generate_suggestions
      end

      it 'uses the conversation FAQ default ahead of the legacy global installation model' do
        create(:installation_config, name: 'TEKOMI_OPEN_AI_MODEL', value: 'gpt-4.1-mini')
      ...
      it 'keeps account conversation FAQ model overrides ahead of the feature default' do
        create(:installation_config, name: 'TEKOMI_OPEN_AI_MODEL', value: 'gpt-4.1')
        conversation.account.update!(tekomi_models: { 'conversation_faq_generation' => 'gpt-4.1-mini' })
      ...
      it 'creates suggestions instead of trusted FAQs for valid conversation content' do
        expect do
          service.generate_suggestions
        end.to change(tekomi_assistant.faq_suggestions, :count).by(2)
        expect(Tekomi::FaqObservation.count).to eq(2)
        expect(tekomi_assistant.responses.count).to be_zero
      end

      it 'saves open suggestions with one attached source each' do
        service.generate_suggestions
        expect(
          tekomi_assistant.faq_suggestions.pluck(:question, :answer, :status, :source_count, :language)
        ).to contain_exactly(
          ['What is the purpose?', 'To help users.', 'open', 1, 'en'],
          ['How does it work?', 'Through AI.', 'open', 1, 'en']
        )
        expect(Tekomi::FaqObservation.attached.pluck(:conversation_id, :language)).to contain_exactly(
    ...
    context 'when finding duplicates' do
      let(:existing_response) do
        create(:tekomi_assistant_response, assistant: tekomi_assistant, account: tekomi_assistant.account,
      ...
      it 'discards candidates the LLM confirms are covered by an approved FAQ' do
        expect do
          service.generate_suggestions
        end.to change(Tekomi::FaqObservation.discarded, :count).by(2)
        expect(tekomi_assistant.faq_suggestions.count).to be_zero
      end
      ...
      it 'uses the account model override for conversation FAQ matching' do
        conversation.account.update!(tekomi_models: { 'conversation_faq_matching' => 'gpt-5-mini' })
    ...
    context 'when the classifier confirms a non-match' do
      ...
      it 'creates a new suggestion' do
        expect do
          service.generate_suggestions
        end.to change(tekomi_assistant.faq_suggestions, :count).by(1)
      end
    end
    ...
    context 'when an open suggestion is the same FAQ' do
      ...
      let(:existing_suggestion) do
        tekomi_assistant.faq_suggestions.create!(
          question: 'How do I enable the feature?',
          answer: 'Turn it on in settings.',
          embedding: embedding_one
        ).tap do |suggestion|
          suggestion.observations.create!(
            conversation: create(:conversation, account: tekomi_assistant.account),
      ...
      it 'attaches the observation and increments the source count' do
        ...
        expect(tekomi_assistant.faq_suggestions.count).to eq(1)
      end
    ...
    context 'when a similar open suggestion uses another language' do
      ...
      let!(:existing_suggestion) do
        tekomi_assistant.faq_suggestions.create!(question: 'How do I enable the feature?', answer: 'Turn it on in settings.',
      ...
      it 'creates a separate suggestion in the conversation language' do
        ...
        expect(tekomi_assistant.faq_suggestions.pluck(:language)).to contain_exactly('en', 'pt')
        expect(existing_suggestion.reload.source_count).to eq(1)
      end
    end
    ...
    context 'when an open suggestion uses another locale variant of the same language' do
      ...
      let(:tekomi_assistant) { create(:tekomi_assistant, account: account) }
    ...
    context 'when a similar approved FAQ uses another language' do
      ...
      it 'deduplicates against the approved FAQ' do
        expect do
          service.generate_suggestions
        end.to change(Tekomi::FaqObservation.discarded, :count).by(1)
        expect(tekomi_assistant.faq_suggestions.count).to be_zero
      end
    end
    ...
    context 'when conversation and account locales share a base language' do
      ...
      let!(:existing_response) do
        create(:tekomi_assistant_response, assistant: tekomi_assistant, account: account,
