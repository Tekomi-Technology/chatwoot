...
RSpec.describe Tekomi::Llm::AssistantFalsePromiseService do
  ...
  describe '#detect' do
    ...
    it 'uses the detector model even when the assistant feature model is overridden' do
      account.update!(tekomi_models: { 'assistant' => 'gpt-5-mini' })
    ...
    it 'uses the false promise schema and detector prompt' do
      expect(mock_chat).to receive(:with_schema).with(Tekomi::AssistantFalsePromiseSchema).and_return(mock_chat)
