...
RSpec.describe Tekomi::Llm::PaginatedFaqGeneratorService do
  let(:document) { create(:tekomi_document) }
  ...
  before do
    ...
    allow(InstallationConfig).to receive(:find_by!)
      .with(name: 'TEKOMI_OPEN_AI_API_KEY')
  ...
  describe '#generate' do
    it 'uses the PDF FAQ generation feature model' do
      document.account.update!(tekomi_models: { 'pdf_faq_generation' => 'gpt-5.2' })
