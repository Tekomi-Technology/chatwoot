...
RSpec.describe Tekomi::Tools::SimplePageCrawlParserJob, type: :job do
  describe '#perform' do
    ...
    let(:crawler) { instance_double(Tekomi::Tools::SimplePageCrawlService) }

    before do
      allow(Tekomi::Tools::SimplePageCrawlService).to receive(:new)
        .with(page_link)
        .and_return(crawler)
    ...
    context 'when the page is successfully crawled' do
      ...
      it 'updates existing document if one exists' do
        existing_document = create(:tekomi_document,
                                   assistant: assistant,
    ...
    context 'when the page fetch fails' do
      ...
      context 'when the failure is permanent' do
        ...
        def run_job
          ...
          raise unless e.class.name == 'Tekomi::Tools::SimplePageCrawlParserJob::PermanentCrawlError' # rubocop:disable Style/ClassEqualityComparison
        end
