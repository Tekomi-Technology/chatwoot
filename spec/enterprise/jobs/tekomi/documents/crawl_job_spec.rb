...
RSpec.describe Tekomi::Documents::CrawlJob, type: :job do
  let(:document) { create(:tekomi_document, external_link: 'https://example.com/page') }
  ...
  describe '#perform' do
    context 'when TEKOMI_FIRECRAWL_API_KEY is configured' do
      ...
      context 'with account usage limits' do
        before do
          allow(account).to receive(:usage_limits).and_return({ tekomi: { documents: { current_available: 20 } } })
        end
      ...
      context 'when crawl limit exceeds maximum' do
        before do
          allow(account).to receive(:usage_limits).and_return({ tekomi: { documents: { current_available: 1000 } } })
        end
    ...
    context 'when TEKOMI_FIRECRAWL_API_KEY is not configured' do
      ...
      before do
        allow(Tekomi::Tools::SimplePageCrawlService)
          .to receive(:new)
      ...
      it 'enqueues SimplePageCrawlParserJob for each discovered link' do
        page_links.each do |link|
          expect(Tekomi::Tools::SimplePageCrawlParserJob)
    ...
    context 'when document is a PDF' do
      let(:pdf_document) do
        doc = create(:tekomi_document, external_link: 'https://example.com/document')
      ...
      it 'processes PDF using PdfProcessingService' do
        pdf_service = instance_double(Tekomi::Llm::PdfProcessingService)
        expect(Tekomi::Llm::PdfProcessingService).to receive(:new).with(pdf_document).and_return(pdf_service)
      ...
      it 'handles PDF processing errors' do
        allow(Tekomi::Llm::PdfProcessingService).to receive(:new).and_raise(StandardError, 'Processing failed')
