...
RSpec.describe Tekomi::Documents::ScheduleSyncsJob, type: :job do
  let(:account) { create(:account, custom_attributes: { plan_name: 'business' }) }
  let(:assistant) { create(:tekomi_assistant, account: account) }

  before do
    set_installation_config('TEKOMI_DOCUMENT_AUTO_SYNC_INTERVALS', { business: 168, enterprise: 24, startups: 720, hacker: nil }.to_json)
    set_installation_config('TEKOMI_DOCUMENT_AUTO_SYNC_PER_ACCOUNT_BATCH_LIMIT', 50)
    set_installation_config('TEKOMI_DOCUMENT_AUTO_SYNC_GLOBAL_BATCH_LIMIT', 1000)
    account.enable_features!('tekomi_document_auto_sync')
    clear_enqueued_jobs
  end
  ...
  def sync_job_for(document)
    have_enqueued_job(Tekomi::Documents::PerformSyncJob).with(document)
  end

  context 'when the account has not enabled auto-sync' do
    before { account.disable_features!('tekomi_document_auto_sync') }

    it 'leaves available documents alone' do
      create(:tekomi_document, assistant: assistant, account: account, status: :available)
      clear_enqueued_jobs

      expect { described_class.new.perform }.not_to have_enqueued_job(Tekomi::Documents::PerformSyncJob)
    end
  end
  ...
  context 'when a plan name is passed' do
    it 'queues due documents only for that plan' do
      ...
      enterprise_account.enable_features!('tekomi_document_auto_sync')
      enterprise_assistant = create(:tekomi_assistant, account: enterprise_account)
      business_document = create(:tekomi_document, assistant: assistant, account: account, status: :available)
      enterprise_document = create(:tekomi_document, assistant: enterprise_assistant, account: enterprise_account, status: :available)
      ...
      expect(Tekomi::Documents::PerformSyncJob).to have_been_enqueued.with(enterprise_document)
      expect(Tekomi::Documents::PerformSyncJob).not_to have_been_enqueued.with(business_document)
    end
  end
  ...
  context 'when an available document has backfilled sync metadata' do
    ...
    it 'delays only the queued sync job' do
      travel_to Time.zone.local(2026, 4, 27, 10, 0, 0) do
        ...
        expect(Tekomi::Documents::PerformSyncJob)
  ...
  context 'when an available document was synced within the plan cadence' do
    it 'leaves it alone' do
      document = create(:tekomi_document, assistant: assistant, account: account, status: :available)
  ...
  context 'when jitter spreads queued sync execution' do
    it 'uses a widened due window so jittered syncs do not skip the next plan run' do
      travel_to Time.zone.local(2026, 4, 27, 10, 0, 0) do
        ...
        expect { job.perform }.not_to have_enqueued_job(Tekomi::Documents::PerformSyncJob)

        document.update!(sync_status: :synced, last_synced_at: (due_window + 1.minute).ago)
        clear_enqueued_jobs

        expect { job.perform }
          .to have_enqueued_job(Tekomi::Documents::PerformSyncJob)
          .with(document)
          .on_queue('purgable')
          .at(2.hours.from_now)
      end
  ...
  context 'when more documents are due than the account cap allows' do
    before do
      update_sync_limit('TEKOMI_DOCUMENT_AUTO_SYNC_PER_ACCOUNT_BATCH_LIMIT', 2)
    end

    it 'queues backfilled and oldest-attempted documents first' do
      newest_document = create(:tekomi_document, assistant: assistant, account: account, status: :available)
      oldest_document = create(:tekomi_document, assistant: assistant, account: account, status: :available)
      backfilled_document = create(:tekomi_document, assistant: assistant, account: account, status: :available)
      ...
      expect(Tekomi::Documents::PerformSyncJob).not_to have_been_enqueued.with(newest_document)
    end
  end
  ...
  context 'when sync caps are configured' do
    it 'uses installation config caps for per-account and global limits' do
      update_sync_limit('TEKOMI_DOCUMENT_AUTO_SYNC_PER_ACCOUNT_BATCH_LIMIT', 2)
      update_sync_limit('TEKOMI_DOCUMENT_AUTO_SYNC_GLOBAL_BATCH_LIMIT', 3)

      second_account = create(:account, custom_attributes: { plan_name: 'business' })
      second_account.enable_features!('tekomi_document_auto_sync')
      second_assistant = create(:tekomi_assistant, account: second_account)

      first_account_documents = create_list(:tekomi_document, 3, assistant: assistant, account: account, status: :available)
      second_account_documents = create_list(:tekomi_document, 3, assistant: second_assistant, account: second_account, status: :available)
      ...
      expect { described_class.new.perform }
        .to have_enqueued_job(Tekomi::Documents::PerformSyncJob).exactly(3).times
    end
  end
  ...
  context 'when a document has been queued longer than the worker lock timeout' do
    it 'leaves it alone so queue lag is not treated as a dead worker' do
      ...
      document.update!(
        sync_status: :syncing,
        last_sync_attempted_at: (Tekomi::Documents::PerformSyncJob::LOCK_TIMEOUT + 1.minute).ago
      )
      clear_enqueued_jobs
  ...
  context 'when the only eligible document is a PDF' do
    it 'leaves it alone since PDFs are not syncable' do
      pdf_document = build(:tekomi_document, assistant: assistant, account: account, status: :available)
  ...
  context 'when an in-progress (still crawling) document is past the cadence' do
    it 'leaves it alone since only available documents are eligible' do
      document = create(:tekomi_document, assistant: assistant, account: account, status: :in_progress)
