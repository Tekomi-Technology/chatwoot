...
RSpec.describe Tekomi::Documents::ResponseBuilderJob, type: :job do
  let(:assistant) { create(:tekomi_assistant) }
  let(:document) { create(:tekomi_document, assistant: assistant) }
  let(:faq_generator) { instance_double(Tekomi::Llm::FaqGeneratorService) }
  let(:faqs) do
    [
  ...
  before do
    allow(Tekomi::Llm::FaqGeneratorService).to receive(:new).with(document: document).and_return(faq_generator)
  ...
  describe '#perform' do
    context 'when processing a document' do
      it 'deletes previous responses' do
        existing_response = create(:tekomi_assistant_response, documentable: document)
      ...
      it 'creates new responses for each FAQ' do
        expect do
          described_class.new.perform(document)
        end.to change(Tekomi::AssistantResponse, :count).by(2)
    ...
    context 'with different locales' do
      ...
      let(:spanish_assistant) { create(:tekomi_assistant, account: spanish_account) }
      let(:spanish_document) { create(:tekomi_document, assistant: spanish_assistant, account: spanish_account) }
      let(:spanish_faq_generator) { instance_double(Tekomi::Llm::FaqGeneratorService) }

      before do
        allow(Tekomi::Llm::FaqGeneratorService).to receive(:new).with(document: spanish_document).and_return(spanish_faq_generator)
      ...
      it 'passes the correct document to FAQ generator' do
        ...
        expect(Tekomi::Llm::FaqGeneratorService).to have_received(:new).with(document: spanish_document)
      end
    end

    context 'when processing a PDF document' do
      let(:pdf_document) do
        doc = create(:tekomi_document, assistant: assistant)
      ...
      let(:paginated_service) { instance_double(Tekomi::Llm::PaginatedFaqGeneratorService) }
      let(:pdf_faqs) do
        [{ 'question' => 'What is in the PDF?', 'answer' => 'Important content' }]
      end

      before do
        allow(Tekomi::Llm::PaginatedFaqGeneratorService).to receive(:new)
      ...
      it 'uses paginated FAQ generator for PDFs' do
        expect(Tekomi::Llm::PaginatedFaqGeneratorService).to receive(:new).with(pdf_document, anything)
