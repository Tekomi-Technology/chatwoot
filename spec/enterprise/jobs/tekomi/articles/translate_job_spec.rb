...
RSpec.describe Tekomi::Articles::TranslateJob, type: :job do
  ...
  let(:title_service) { instance_double(Tekomi::Llm::ArticleTranslationService) }
  let(:content_service) { instance_double(Tekomi::Llm::ArticleTranslationService) }

  before do
    allow(Tekomi::Llm::ArticleTranslationService).to receive(:new).with(hash_including(type: :title)).and_return(title_service)
    allow(Tekomi::Llm::ArticleTranslationService).to receive(:new).with(hash_including(type: :content)).and_return(content_service)
  ...
  it 'calls the translation service with the correct language' do
    ...
    expect(Tekomi::Llm::ArticleTranslationService).to have_received(:new).with(
