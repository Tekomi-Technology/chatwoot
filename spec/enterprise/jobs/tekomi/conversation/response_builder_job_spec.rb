...
RSpec.describe Tekomi::Conversation::ResponseBuilderJob, type: :job do
  ...
  describe '#perform' do
    ...
    let(:mock_llm_chat_service) { instance_double(Tekomi::Llm::AssistantChatService) }
    let(:mock_agent_runner_service) { instance_double(Tekomi::Assistant::AgentRunnerService) }
    let(:mock_action_classifier_service) { instance_double(Tekomi::Llm::AssistantActionClassifierService) }
    let(:mock_false_promise_service) { instance_double(Tekomi::Llm::AssistantFalsePromiseService) }
    ...
    let(:v2_response_parts) { [{ 'text' => 'Hey, welcome to Tekomi V2', 'citation_indexes' => [] }] }

    before do
      allow(ChatwootApp).to receive(:chatwoot_cloud?).and_return(true)
      allow(inbox).to receive(:tekomi_active?).and_return(true)
      allow(Tekomi::Llm::AssistantChatService).to receive(:new).and_return(mock_llm_chat_service)
      allow(mock_llm_chat_service).to receive(:generate_response).and_return({ 'response' => 'Hey, welcome to Tekomi Specs' })
      allow(Tekomi::Assistant::AgentRunnerService).to receive(:new).and_return(mock_agent_runner_service)
      allow(mock_agent_runner_service).to receive(:generate_response).and_return({
                                                                                   'response_parts' => v2_response_parts,
                                                                                   'response' => 'Hey, welcome to Tekomi V2',
      ...
      allow(Tekomi::Llm::AssistantActionClassifierService).to receive(:new).and_return(mock_action_classifier_service)
      allow(mock_action_classifier_service).to receive(:classify).and_return({ 'action' => 'continue' })
      allow(Tekomi::Llm::AssistantFalsePromiseService).to receive(:new).and_return(mock_false_promise_service)
      allow(mock_false_promise_service).to receive(:detect).and_return({ 'decision' => 'safe', 'reason' => 'safe_response' })
    end

    context 'when tekomi_v2 is disabled' do
      before do
        allow(account).to receive(:feature_enabled?).and_return(false)
        allow(account).to receive(:feature_enabled?).with('tekomi_integration_v2').and_return(false)
      end

      it 'uses Tekomi::Llm::AssistantChatService' do
        expect(Tekomi::Llm::AssistantChatService).to receive(:new).with(assistant: assistant, conversation: conversation)
        expect(Tekomi::Assistant::AgentRunnerService).not_to receive(:new)

        described_class.perform_now(conversation, assistant)
        expect(conversation.messages.last.content).to eq('Hey, welcome to Tekomi Specs')
      end
      ...
      it 'does not emit tekomi lifecycle events' do
        expect(Tekomi::ConversationEvents).not_to receive(:response_completed)
      ...
      it 'keeps the default message history limited to public chat messages' do
        ...
        ).and_return({ 'response' => 'Hey, welcome to Tekomi Specs' })
      ...
      it 'increments usage response' do
        described_class.perform_now(conversation, assistant)
        account.reload
        expect(account.usage_limits[:tekomi][:responses][:consumed]).to eq(1)
      end

      it 'does not create a tekomi session' do
        expect do
          described_class.perform_now(conversation, assistant)
        end.not_to change(Tekomi::AgentSession, :count)
      end

      it 'does not run the action classifier when the classifier feature is disabled' do
        expect(Tekomi::Llm::AssistantActionClassifierService).not_to receive(:new)
      ...
      it 'does not run the false promise harness when the account setting is disabled' do
        expect(Tekomi::Llm::AssistantFalsePromiseService).not_to receive(:new)
      ...
      context 'when false promise harness is enabled in account settings' do
        before do
          account.update!(settings: account.settings.merge('tekomi_false_promise_harness_enabled' => true))
        end
        ...
        it 'sends the original response when the detector marks it safe' do
          expect(mock_false_promise_service).to receive(:detect).with(
            message_history: [{ content: 'Hello', role: 'user' }],
            assistant_response: 'Hey, welcome to Tekomi Specs'
          ).and_return({
                         'decision' => 'safe',
          ...
          expect(conversation.messages.outgoing.last.content).to eq('Hey, welcome to Tekomi Specs')
          expect(account.reload.usage_limits[:tekomi][:responses][:consumed]).to eq(1)
        end
        ...
        it 'regenerates future-work promises through the V1 assistant chat service' do
          ...
          expect(mock_llm_chat_service).to have_received(:generate_response).with(
            ...
            additional_message: Tekomi::Conversation::V1FalsePromiseHandler::FUTURE_PROMISE_REPAIR_INSTRUCTION
          )
        end
        ...
        it 'hands off instead of sending the unsafe draft when repair generation fails' do
          ...
          expect(conversation.messages.outgoing.last.content).to eq(I18n.t('conversations.tekomi.handoff'))
          expect(conversation.messages.outgoing.pluck(:content)).not_to include('Let me check and get back to you.')
          expect(account.reload.usage_limits[:tekomi][:responses][:consumed]).to eq(0)
        end
        ...
        it 'skips the false promise harness when the action classifier already requested handoff' do
          allow(account).to receive(:feature_enabled?).and_return(false)
          allow(account).to receive(:feature_enabled?).with('tekomi_integration_v2').and_return(false)
          allow(account).to receive(:feature_enabled?).with('tekomi_v1_action_classifier').and_return(true)
      ...
      context 'when V1 action classifier is enabled' do
        ...
        it 'keeps the conversation pending when the classifier returns continue' do
          expect(Tekomi::Llm::AssistantActionClassifierService).to receive(:new).with(
            assistant: assistant,
            conversation: conversation
        ...
        it 'skips the classifier when the conversation is no longer pending after response generation' do
          allow(mock_llm_chat_service).to receive(:generate_response) do
            conversation.open!
            { 'response' => 'Hey, welcome to Tekomi Specs' }
          end
    ...
    context 'when tekomi_v2 is enabled' do
      before do
        allow(account).to receive(:feature_enabled?).and_return(false)
        allow(account).to receive(:feature_enabled?).with('tekomi_integration_v2').and_return(true)
      end

      context 'with message burst protection' do
        it 'passes the responding message id to the runner' do
          expect(Tekomi::Assistant::AgentRunnerService).to receive(:new).with(
            assistant: assistant,
            conversation: conversation,
        ...
        it 'emits only the response failed event when generation raises after a newer message arrived' do
          ...
          expect(Tekomi::ConversationEvents).to receive(:response_failed)
            .with(conversation: conversation, assistant: assistant, reason: 'standard_error', at: kind_of(Time))
          expect(Tekomi::ConversationEvents).not_to receive(:handed_off)
        ...
        it 'keeps the pending response fresh when an email auto reply arrives' do
          ...
          expect(conversation.messages.outgoing.last.content).to eq('Hey, welcome to Tekomi V2')
          expect(account.reload.usage_limits[:tekomi][:responses][:consumed]).to eq(1)
        end
      end
      ...
      it 'passes message history with resolution markers to agent runner service' do
        ...
        create(
          ...
          additional_attributes: {
            Tekomi::Assistant::ResponseParts::MESSAGE_ATTRIBUTE_KEY => [
        ...
        expected_messages = [
          ...
          {
            content: Tekomi::Conversation::MessageHistoryBuilderService::RESOLUTION_MARKER,
            role: 'assistant'
          },
      ...
      it 'does not restore deleted Tekomi response parts into message history' do
      ...
      it 'generates and processes response' do
        ...
        expect(conversation.messages.count).to eq(2)
        expect(conversation.messages.outgoing.count).to eq(1)
        expect(conversation.messages.last.content).to eq('Hey, welcome to Tekomi V2')
        expect(
          conversation.messages.last.additional_attributes[Tekomi::Assistant::ResponseParts::MESSAGE_ATTRIBUTE_KEY]
      ...
      context 'with structured citations' do
        before do
          ...
          first_document = create(:tekomi_document, assistant: assistant, external_link: 'https://help.example.com/password')
          second_document = create(:tekomi_document, assistant: assistant, external_link: 'https://help.example.com/email')
          run_result = Agents::RunResult.new(
            output: { 'response_parts' => v2_response_parts, 'reasoning' => 'Used the FAQ results' },
            context: {
              state: {
                Tekomi::Assistant::CITATION_SOURCES_STATE_KEY => {
                  1 => first_document.id,
        ...
        it 'renders one cited document from the trusted run mapping' do
          ...
          expect(conversation.messages.outgoing.last.content).to eq(
            'Hey, welcome to Tekomi V2 [[1](https://help.example.com/password)]'
          )
          cited_document_id = mock_agent_runner_service.last_run_result.context[:state][Tekomi::Assistant::CITATION_SOURCES_STATE_KEY][1]
          expect(Tekomi::AgentSession.last.cited_document_ids).to eq([cited_document_id])
        end
        ...
        it 'encodes Markdown delimiters in citation URLs' do
          ...
          citation_sources = mock_agent_runner_service.last_run_result.context[:state][Tekomi::Assistant::CITATION_SOURCES_STATE_KEY]
          ...
          expect(content).to eq('Hey, welcome to Tekomi V2 [[1](https://help.example.com/guides/reset_%28new%29)]')
        ...
        it 'renders a wrong but valid index only through its trusted source mapping' do
          ...
          content = conversation.messages.outgoing.last.content
          expect(content).to eq('Hey, welcome to Tekomi V2 [[1](https://help.example.com/email)]')
        ...
        it 'joins valid markdown parts and drops blank or invalid parts' do
          ...
          expect(message.additional_attributes[Tekomi::Assistant::ResponseParts::MESSAGE_ATTRIBUTE_KEY]).to eq(
            [
              { 'text' => "**Steps**\n\n- Open settings", 'citation_indexes' => [] },
      ...
      it 'emits a response completed event' do
        expect(Tekomi::ConversationEvents).to receive(:response_completed)
    ...
    context 'when tekomi_v2 handoff tool fires during agent execution' do
      before do
        allow(account).to receive(:feature_enabled?).and_return(false)
        allow(account).to receive(:feature_enabled?).with('tekomi_integration_v2').and_return(true)
      ...
      it 'creates a public handoff message after the generated response is discarded' do
        ...
        expect(public_messages.last.content).to eq(I18n.t('conversations.tekomi.handoff'))
      end
    ...
    context 'when capturing assistant sessions' do
      let(:run_context) do
        {
          ...
          conversation_history: [
            ...
            { role: :assistant, content: 'Hey, welcome to Tekomi V2', agent_name: 'Assistant' }
          ],
      ...
      let(:run_result) { Agents::RunResult.new(output: { 'response' => 'Hey, welcome to Tekomi V2' }, usage: usage, context: run_context) }
      ...
      it 'creates a session for a delivered response' do
        ...
        session = Tekomi::AgentSession.last
      ...
      it 'still delivers the reply when session capture fails' do
        allow(Tekomi::AgentSession).to receive(:create!).and_raise(StandardError, 'capture failed')
        allow(ChatwootExceptionTracker).to receive(:new).and_call_original
  ...
  describe 'retry mechanisms for image processing' do
    let(:conversation) { create(:conversation, inbox: inbox, account: account, status: :pending) }
    let(:mock_llm_chat_service) { instance_double(Tekomi::Llm::AssistantChatService) }
    let(:mock_message_builder) { instance_double(Tekomi::OpenAiMessageBuilderService) }

    before do
      create(:message, conversation: conversation, content: 'Hello with image', message_type: :incoming)
      allow(Tekomi::Llm::AssistantChatService).to receive(:new).and_return(mock_llm_chat_service)
      allow(Tekomi::OpenAiMessageBuilderService).to receive(:new).with(message: anything).and_return(mock_message_builder)
