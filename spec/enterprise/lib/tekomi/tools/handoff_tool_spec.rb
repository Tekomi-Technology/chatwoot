...
RSpec.describe Tekomi::Tools::HandoffTool, type: :model do
  ...
  describe '#perform' do
    context 'when conversation exists' do
      context 'when Tekomi is responding to a customer message' do
        ...
        it 'hands off when no newer customer message has arrived' do
          ...
          expect(tool_context.state[:tekomi_v2_handoff_tool_completed]).to be true
        end
        ...
        it 'emits a tekomi handoff event with the tool source after the locked handoff completes' do
        ...
        it 'does not emit a tekomi handoff event when the handoff is skipped as stale' do
          create(:message, conversation: conversation, account: account, inbox: inbox, message_type: :incoming)
      ...
      context 'with Tekomi V1' do
        ...
        it 'uses the legacy handoff without a lock or stale-message guard' do
          ...
          expect(tool_context.state).not_to have_key(:tekomi_v2_handoff_tool_completed)
        end
      end
      ...
      context 'with reason provided' do
        ...
        it 'emits a tekomi handoff event with the tool source and reason category' do
          expect(Tekomi::ConversationEvents).to receive(:handed_off)
