...
RSpec.describe Tekomi::Tools::FaqLookupTool, type: :model do
  ...
  before do
    ...
    embedding_service = instance_double(Tekomi::Llm::EmbeddingService)
  ...
  describe '#perform' do
    context 'when FAQs exist' do
      ...
      before do
        ...
        allow(Tekomi::AssistantResponse).to receive(:nearest_neighbors).and_return(
          Tekomi::AssistantResponse.where(id: [response1.id, response2.id])
        )
      end
      ...
      it 'does not assign indexes to attachments or storage links' do
        ...
        expect(tool_context.state[Tekomi::Assistant::CITATION_SOURCES_STATE_KEY]).to be_nil
      ...
      it 'assigns stable numeric indexes to customer-visible document links' do
        ...
        expect(tool_context.state[Tekomi::Assistant::CITATION_SOURCES_STATE_KEY]).to eq(1 => document.id)
      end
      ...
      it 'records retrieved faq ids and document ids into Chatwoot metadata' do
        ...
        user_faq = create(:tekomi_assistant_response, assistant: assistant, documentable: user, status: :approved)
        allow(Tekomi::AssistantResponse).to receive(:nearest_neighbors).and_return(
          Tekomi::AssistantResponse.where(id: user_faq.id)
        )
    ...
    context 'when no FAQs found' do
      before do
        ...
        allow(Tekomi::AssistantResponse).to receive(:nearest_neighbors).and_return(Tekomi::AssistantResponse.none)
      end
