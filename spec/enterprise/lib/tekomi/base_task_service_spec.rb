...
RSpec.describe Tekomi::BaseTaskService, type: :model do
  ...
  let(:test_service_class) do
    ...
    klass.prepend(Enterprise::Tekomi::BaseTaskService)
    klass
  end
  ...
  describe '#perform with enterprise usage tracking' do
    # Ensure tekomi is enabled by default for tests unless explicitly testing disabled state
    before do
    ...
    context 'when usage limit is exceeded' do
      ...
      it 'returns usage limit exceeded error' do
        ...
        expect(result[:error]).to eq(I18n.t('tekomi.copilot_limit'))
    ...
    it 'actually increments the usage counter in custom_attributes' do
      expect do
        service.perform
        account.reload
      end.to change { account.custom_attributes['tekomi_responses_usage'].to_i }.by(1)
    end
    ...
    context 'when account has its own OpenAI hook key' do
      ...
      context 'when the tekomi_responses quota is exhausted on Cloud' do
        before do
          allow(ChatwootApp).to receive(:chatwoot_cloud?).and_return(true)
          allow(account).to receive(:usage_limits).and_return(exhausted_usage_limits)
        end
    ...
    context 'when tekomi is disabled' do
      before do
      ...
      context 'when on Chatwoot Cloud' do
        ...
        it 'returns upgrade error message' do
          result = service.perform
          expect(result[:error]).to eq(I18n.t('tekomi.upgrade'))
        end
    ...
    context 'when tekomi is enabled' do
      before do
        allow(account).to receive(:feature_enabled?).with('tekomi_tasks').and_return(true)
      end
