...
RSpec.describe Tekomi::PromptRenderer do
  let(:template_name) { 'test_template' }
  let(:template_content) { 'Hello {{name}}, your balance is {{balance}}' }
  let(:template_path) { Rails.root.join('enterprise', 'lib', 'tekomi', 'prompts', "#{template_name}.liquid") }
  ...
  describe 'snippet rendering' do
    let(:snippets_dir) { Rails.root.join('enterprise/lib/tekomi/prompts/snippets') }
  ...
  describe '.load_template' do
    ...
    it 'constructs correct template path' do
      expected_path = Rails.root.join('enterprise/lib/tekomi/prompts/my_template.liquid')
