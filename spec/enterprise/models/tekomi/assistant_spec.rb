...
RSpec.describe Tekomi::Assistant, type: :model do
  ...
  describe 'inactive conversation settings' do
    it 'uses safe defaults when settings are unavailable' do
      assistant.account.enable_features('tekomi_integration_v2')
      ...
      assistant.account.disable_features('tekomi_integration_v2')
  ...
  describe '#auto_resolve_mode' do
    let(:account) { create(:account, tekomi_auto_resolve_mode: 'legacy') }
    ...
    it 'rejects unsupported modes' do
      assistant = build(:tekomi_assistant, account: account, config: { 'auto_resolve_mode' => 'unsupported' })
  ...
  describe '#agent_tools' do
    it 'includes enabled custom tools from the assistant account' do
      custom_tool = create(:tekomi_custom_tool, account: account)

      tools = assistant.send(:agent_tools)

      expect(tools.map(&:name)).to include(custom_tool.slug)
      expect(tools.find { |tool| tool.name == custom_tool.slug }).to be_a(Tekomi::Tools::HttpTool)
    end

    it 'excludes disabled custom tools' do
      custom_tool = create(:tekomi_custom_tool, :disabled, account: account)
    ...
    it 'excludes custom tools from other accounts' do
      custom_tool = create(:tekomi_custom_tool)
    ...
    it 'keeps the built-in FAQ lookup and handoff tools' do
      ...
      expect(tools).to include(
        an_instance_of(Tekomi::Tools::FaqLookupTool),
        an_instance_of(Tekomi::Tools::HandoffTool)
      )
    end
  end
  ...
  describe '#agent_instructions' do
    it 'keeps the Assistant human handoff prompt unchanged' do
      ...
      expect(instructions).to include('# Human Handoff Protocol', 'tekomi--tools--handoff')
    ...
    it 'renders the separate Copilot reply suggestion prompt when requested' do
      ...
      scenario = create(:tekomi_scenario, assistant: assistant, account: account, title: 'Refund workflow')
