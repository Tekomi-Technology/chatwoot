...
RSpec.describe Tekomi::MessageReport, type: :model do
  describe 'associations' do
    ...
    it 'resolves the conversation association to the top-level Conversation model' do
      # `Tekomi::Conversation` exists as a job namespace, so without an explicit
  ...
  describe 'factory' do
    it 'creates a valid message report' do
      expect(build(:tekomi_message_report)).to be_valid
    end
  end
end
