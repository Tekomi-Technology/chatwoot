...
RSpec.describe Tekomi::AssistantResponse, type: :model do
  describe 'account validation' do
    it 'uses the assistant account when the account is not set' do
      assistant = create(:tekomi_assistant)
      assistant_response = build(:tekomi_assistant_response, assistant: assistant, account: nil)
    ...
    it 'rejects an assistant from another account' do
      ...
      assistant_response = build(:tekomi_assistant_response, account: account)
