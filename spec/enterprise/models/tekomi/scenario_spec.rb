...
RSpec.describe Tekomi::Scenario, type: :model do
  ...
  describe 'scopes' do
    ...
    describe '.enabled' do
      it 'returns only enabled scenarios' do
        enabled_scenario = create(:tekomi_scenario, assistant: assistant, account: account, enabled: true)
        disabled_scenario = create(:tekomi_scenario, assistant: assistant, account: account, enabled: false)
  ...
  describe 'callbacks' do
    ...
    describe 'before_save :resolve_tool_references' do
      it 'calls resolve_tool_references before saving' do
        scenario = build(:tekomi_scenario, assistant: assistant, account: account)
  ...
  describe '#handoff_key' do
    ...
    it 'uses id plus readable slug for persisted scenarios' do
      scenario = create(:tekomi_scenario, assistant: assistant, account: account,
  ...
  describe 'tool validation and population' do
    ...
    describe 'validate_instruction_tools' do
      it 'is valid with valid tool references' do
        scenario = build(:tekomi_scenario,
                         assistant: assistant,
                         account: account,
      ...
      it 'is valid with custom tool references' do
        create(:tekomi_custom_tool, account: account, slug: 'custom_fetch-order')
        scenario = build(:tekomi_scenario,
                         assistant: assistant,
                         account: account,
      ...
      it 'is invalid with custom tool from different account' do
        ...
        create(:tekomi_custom_tool, account: other_account, slug: 'custom_fetch-order')
        scenario = build(:tekomi_scenario,
                         assistant: assistant,
                         account: account,
                         instruction: 'Use [@Fetch Order](tool://custom_fetch-order) to get order details')
      ...
      it 'is invalid with disabled custom tool' do
        create(:tekomi_custom_tool, account: account, slug: 'custom_fetch-order', enabled: false)
        scenario = build(:tekomi_scenario,
                         assistant: assistant,
                         account: account,
                         instruction: 'Use [@Fetch Order](tool://custom_fetch-order) to get order details')
    ...
    describe 'resolve_tool_references' do
      it 'populates tools array with referenced tool IDs' do
        scenario = create(:tekomi_scenario,
                          assistant: assistant,
                          account: account,
  ...
  describe 'custom tool integration' do
    ...
    describe '#resolved_tools' do
      ...
      it 'excludes disabled custom tools' do
        custom_tool = create(:tekomi_custom_tool, account: account, slug: 'custom_fetch-order', enabled: true)
        scenario = create(:tekomi_scenario,
                          assistant: assistant,
                          account: account,
                          instruction: 'Use [@Fetch Order](tool://custom_fetch-order)')
    ...
    describe '#resolve_tool_instance' do
      ...
      it 'returns static tool instance for non-custom tools' do
        ...
        expect(tool_instance).not_to be_a(Tekomi::Tools::HttpTool)
      end
    end
    ...
    describe '#agent_tools' do
      it 'returns array of tool instances including custom tools' do
        ...
        expect(tools.first).to be_a(Tekomi::Tools::HttpTool)
      end
      ...
      it 'returns mixed static and custom tool instances' do
        ...
        expect(tools.last).to be_a(Tekomi::Tools::HttpTool)
      end
    end
  end
