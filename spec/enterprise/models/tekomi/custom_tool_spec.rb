...
RSpec.describe Tekomi::CustomTool, type: :model do
  ...
  describe 'validations' do
    ...
    describe 'slug uniqueness' do
      ...
      it 'validates uniqueness of slug scoped to account' do
        create(:tekomi_custom_tool, account: account, slug: 'custom_test_tool')
        duplicate = build(:tekomi_custom_tool, account: account, slug: 'custom_test_tool')
      ...
      it 'allows same slug across different accounts' do
        ...
        create(:tekomi_custom_tool, account: account, slug: 'custom_test_tool')
        different_account_tool = build(:tekomi_custom_tool, account: account2, slug: 'custom_test_tool')
    ...
    describe 'param_schema validation' do
      ...
      it 'is valid with proper param_schema' do
        tool = build(:tekomi_custom_tool, account: account, param_schema: [
  ...
  describe 'scopes' do
    ...
    describe '.enabled' do
      it 'returns only enabled custom tools' do
        enabled_tool = create(:tekomi_custom_tool, account: account, enabled: true)
        disabled_tool = create(:tekomi_custom_tool, account: account, enabled: false)
  ...
  describe 'slug generation' do
    ...
    it 'generates slug from title on creation' do
      tool = create(:tekomi_custom_tool, account: account, title: 'Fetch Order Status')

      expect(tool.slug).to eq('custom_fetch_order_status')
    end

    it 'adds custom_ prefix to generated slug' do
      tool = create(:tekomi_custom_tool, account: account, title: 'My Tool')

      expect(tool.slug).to start_with('custom_')
    end

    it 'does not override manually set slug' do
      tool = create(:tekomi_custom_tool, account: account, title: 'Test Tool', slug: 'custom_manual_slug')

      expect(tool.slug).to eq('custom_manual_slug')
    end

    it 'handles slug collisions by appending random suffix' do
      create(:tekomi_custom_tool, account: account, title: 'Test Tool', slug: 'custom_test_tool')
      tool2 = create(:tekomi_custom_tool, account: account, title: 'Test Tool')
    ...
    it 'handles multiple slug collisions' do
      create(:tekomi_custom_tool, account: account, title: 'Test Tool', slug: 'custom_test_tool')
      create(:tekomi_custom_tool, account: account, title: 'Test Tool', slug: 'custom_test_tool_abc123')
      tool3 = create(:tekomi_custom_tool, account: account, title: 'Test Tool')
    ...
    it 'does not generate slug when title is blank' do
      tool = build(:tekomi_custom_tool, account: account, title: nil)
    ...
    it 'parameterizes title correctly' do
      tool = create(:tekomi_custom_tool, account: account, title: 'Fetch Order Status & Details!')
  ...
  describe 'factory' do
    it 'creates a valid custom tool with default attributes' do
      tool = create(:tekomi_custom_tool)
    ...
    it 'creates valid tool with POST trait' do
      tool = create(:tekomi_custom_tool, :with_post)
    ...
    it 'creates valid tool with bearer auth trait' do
      tool = create(:tekomi_custom_tool, :with_bearer_auth)
    ...
    it 'creates valid tool with basic auth trait' do
      tool = create(:tekomi_custom_tool, :with_basic_auth)
    ...
    it 'creates valid tool with api key trait' do
      tool = create(:tekomi_custom_tool, :with_api_key)
  ...
  describe 'Toolable concern' do
    ...
    describe '#build_request_url' do
      it 'returns static URL when no template variables present' do
        tool = create(:tekomi_custom_tool, account: account, endpoint_url: 'https://api.example.com/orders')

        expect(tool.build_request_url({})).to eq('https://api.example.com/orders')
      end

      it 'renders URL template with params' do
        tool = create(:tekomi_custom_tool, account: account, endpoint_url: 'https://api.example.com/orders/{{ order_id }}')
      ...
      it 'handles multiple template variables' do
        tool = create(:tekomi_custom_tool, account: account,
    ...
    describe '#build_request_body' do
      it 'returns nil when request_template is blank' do
        tool = create(:tekomi_custom_tool, account: account, request_template: nil)
    ...
    describe '#format_response' do
      it 'returns raw response when no response_template' do
        tool = create(:tekomi_custom_tool, account: account, response_template: nil)
    ...
    describe '#build_metadata_headers' do
      let(:tool) { create(:tekomi_custom_tool, account: account, slug: 'custom_test_tool') }
    ...
    describe '#tool' do
      ...
      it 'returns HttpTool instance' do
        ...
        expect(tool_instance).to be_a(Tekomi::Tools::HttpTool)
      end

      it 'sets description on the tool class' do
        tool = create(:tekomi_custom_tool, account: account, description: 'Fetches order data')
      ...
      it 'sets parameters on the tool class' do
        tool = create(:tekomi_custom_tool, :with_params, account: account)
      ...
      it 'works with empty param_schema' do
        tool = create(:tekomi_custom_tool, account: account, param_schema: [])
