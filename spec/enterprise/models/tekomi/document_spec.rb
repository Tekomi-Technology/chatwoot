...
RSpec.describe Tekomi::Document, type: :model do
  ...
  describe '#customer_visible_source_url' do
    let(:document) do
      build(:tekomi_document, assistant: assistant, account: account, external_link: 'https://example.com/runbooks')
    end
  ...
  describe 'PDF support' do
    let(:pdf_document) do
      doc = build(:tekomi_document, assistant: assistant, account: account)
      doc.pdf_file.attach(
    ...
    describe 'validations' do
      ...
      it 'validates PDF file size' do
        ...
        expect(doc.errors[:pdf_file]).to include(I18n.t('tekomi.documents.pdf_size_error'))
      end
    end
    ...
    describe '#pdf_document?' do
      ...
      it 'returns true for .pdf external links' do
        doc = build(:tekomi_document, external_link: 'https://example.com/document.pdf')
        expect(doc.pdf_document?).to be true
      end

      it 'returns true for PDF:-prefixed external links even when the blob is missing' do
        doc = build(:tekomi_document, external_link: 'PDF: report_20250101120000')
        expect(doc.pdf_document?).to be true
      end

      it 'returns false for non-PDF documents' do
        doc = build(:tekomi_document, external_link: 'https://example.com')
    ...
    describe '#display_url' do
      ...
      it 'returns external_link for web documents' do
        doc = create(:tekomi_document, external_link: 'https://example.com')
  ...
  describe 'response builder job callback' do
    ...
    describe 'non-PDF documents' do
      it 'enqueues when created with available status and content' do
        ...
        end.to have_enqueued_job(Tekomi::Documents::ResponseBuilderJob)
      end

      it 'does not enqueue when created available without content' do
        expect do
          create(:tekomi_document, assistant: assistant, account: account, status: :available, content: nil)
        end.not_to have_enqueued_job(Tekomi::Documents::ResponseBuilderJob)
      end
