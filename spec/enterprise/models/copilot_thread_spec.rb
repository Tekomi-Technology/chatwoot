...
RSpec.describe CopilotThread, type: :model do
  ...
  describe '#push_event_data' do
    ...
    let(:copilot_thread) { create(:tekomi_copilot_thread, account: account, user: user, assistant: assistant, title: 'Test Thread') }
  ...
  describe '#previous_history' do
    ...
    context 'when there are messages in the thread' do
      before do
        create(:tekomi_copilot_message, copilot_thread: copilot_thread, message_type: 'user', message: { 'content' => 'User message' })
        create(:tekomi_copilot_message, copilot_thread: copilot_thread, message_type: 'assistant_thinking', message: { 'content' => 'Thinking...' })
        create(:tekomi_copilot_message, copilot_thread: copilot_thread, message_type: 'assistant', message: { 'content' => 'Assistant message' })
      end
