...
RSpec.describe Concerns::TekomiToolsHelpers, type: :concern do
  ...
  describe 'TOOL_REFERENCE_REGEX' do
    it 'matches tool references in text' do
      ...
      matches = text.scan(Concerns::TekomiToolsHelpers::TOOL_REFERENCE_REGEX)
    ...
    it 'does not match invalid formats' do
      ...
      invalid_formats.each do |format|
        matches = format.scan(Concerns::TekomiToolsHelpers::TOOL_REFERENCE_REGEX)
  ...
  describe '.resolve_tool_class' do
    it 'resolves valid tool classes' do
      ...
      stub_const('Tekomi::Tools::AddContactNoteTool', Class.new)

      result = test_class.resolve_tool_class('add_contact_note')
      expect(result).to eq(Tekomi::Tools::AddContactNoteTool)
    end
    ...
    it 'converts snake_case to PascalCase' do
      stub_const('Tekomi::Tools::AddPrivateNoteTool', Class.new)

      result = test_class.resolve_tool_class('add_private_note')
      expect(result).to eq(Tekomi::Tools::AddPrivateNoteTool)
    end
  end
