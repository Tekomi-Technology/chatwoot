...
RSpec.describe 'Api::V1::Accounts::Tekomi::AgentSessions', type: :request do
  ...
  describe 'GET /api/v1/accounts/:account_id/tekomi/agent_sessions/:id' do
    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        get "/api/v1/accounts/#{account.id}/tekomi/agent_sessions/#{message.id}", as: :json
    ...
    context 'when the message has an agent session' do
      let(:document) { create(:tekomi_document, account: account, assistant: assistant, name: 'Password reset guide') }
      let(:documented_faq) do
        create(:tekomi_assistant_response, account: account, assistant: assistant,
      ...
      let(:scenario) { create(:tekomi_scenario, account: account, assistant: assistant, title: 'Refund flow') }
      let(:run_context) do
        [
      ...
      it 'returns the session with hydrated citations and scenarios' do
        get "/api/v1/accounts/#{account.id}/tekomi/agent_sessions/#{message.id}",
    ...
    context 'when the message does not belong to the account' do
      it 'returns not found' do
        ...
        get "/api/v1/accounts/#{account.id}/tekomi/agent_sessions/#{other_message.id}",
            headers: agent.create_new_auth_token, as: :json
