...
RSpec.describe 'Api::V1::Accounts::Tekomi::AssistantResponses', type: :request do
  let(:account) { create(:account) }
  let(:assistant) { create(:tekomi_assistant, account: account) }
  let(:document) { create(:tekomi_document, assistant: assistant, account: account) }
  let(:admin) { create(:user, account: account, role: :administrator) }
  let(:agent) { create(:user, account: account, role: :agent) }
  let(:another_assistant) { create(:tekomi_assistant, account: account) }
  let(:another_document) { create(:tekomi_document, account: account, assistant: assistant) }

  def json_response
    JSON.parse(response.body, symbolize_names: true)
  end

  describe 'GET /api/v1/accounts/:account_id/tekomi/assistant_responses' do
    context 'when showing conversation usage' do
      let!(:manual_response) do
        create(:tekomi_assistant_response, account: account, assistant: assistant, documentable: admin)
      end
      let!(:approved_suggestion) do
        create(:tekomi_assistant_response, account: account, assistant: assistant, documentable: nil)
      end
      let!(:document_response) do
        create(:tekomi_assistant_response, account: account, assistant: assistant, documentable: document)
      end
      ...
      before do
        ...
        create(:tekomi_agent_session, account: account, assistant: another_assistant,
                                       subject: second_conversation, used_faq_ids: [manual_response.id], credits_consumed: 1.0)
      end

      it 'returns distinct conversation usage only for user-created FAQs to an admin' do
        get "/api/v1/accounts/#{account.id}/tekomi/assistant_responses",
      ...
      it 'excludes handoff sessions from manual FAQ usage' do
        handoff_response = create(:tekomi_assistant_response, account: account, assistant: assistant, documentable: admin)
        ...
        get "/api/v1/accounts/#{account.id}/tekomi/assistant_responses/#{handoff_response.id}/drilldown",
            headers: admin.create_new_auth_token,
            as: :json
    ...
    context 'when no filters are applied' do
      before do
        create_list(:tekomi_assistant_response, 30,
                    account: account,
                    assistant: assistant,
    ...
    context 'when filtering by assistant_id' do
      before do
        create_list(:tekomi_assistant_response, 3,
                    account: account,
                    assistant: assistant,
                    documentable: document)
    ...
    context 'when searching' do
      before do
        create(:tekomi_assistant_response,
               account: account,
               assistant: assistant,
  ...
  describe 'GET /api/v1/accounts/:account_id/tekomi/assistant_responses/:id' do
    let!(:response_record) { create(:tekomi_assistant_response, assistant: assistant, account: account) }

    it 'returns the requested response if the user is agent or admin' do
      get "/api/v1/accounts/#{account.id}/tekomi/assistant_responses/#{response_record.id}",
          headers: agent.create_new_auth_token,
          as: :json
  ...
  describe 'GET /api/v1/accounts/:account_id/tekomi/assistant_responses/:id/drilldown' do
    let!(:manual_response) do
      create(:tekomi_assistant_response, account: account, assistant: assistant, documentable: admin)
    end
    let!(:approved_suggestion) do
    ...
    it 'returns unauthorized for an agent' do
      get "/api/v1/accounts/#{account.id}/tekomi/assistant_responses/#{manual_response.id}/drilldown",
          headers: agent.create_new_auth_token,
          as: :json
    ...
    it 'does not expose drilldown for an approved FAQ suggestion' do
      get "/api/v1/accounts/#{account.id}/tekomi/assistant_responses/#{approved_suggestion.id}/drilldown",
          headers: admin.create_new_auth_token,
          as: :json
  ...
  describe 'POST /api/v1/accounts/:account_id/tekomi/assistant_responses' do
    let(:valid_params) do
      {
        assistant_response: {
    ...
    it 'creates a new response if the user is an admin' do
      expect do
        post "/api/v1/accounts/#{account.id}/tekomi/assistant_responses",
             params: valid_params,
             headers: admin.create_new_auth_token,
             as: :json
      end.to change(Tekomi::AssistantResponse, :count).by(1)
    ...
    it 'does not accept the removed pending status' do
      ...
      expect(response).to have_http_status(:success)
      expect(Tekomi::AssistantResponse.last).to be_approved
    end

    it 'does not create a response for an assistant in another account' do
      other_assistant = create(:tekomi_assistant)
      ...
      expect do
        post "/api/v1/accounts/#{account.id}/tekomi/assistant_responses",
             ...
             headers: admin.create_new_auth_token,
             as: :json
      end.not_to change(Tekomi::AssistantResponse, :count)
  ...
  describe 'PATCH /api/v1/accounts/:account_id/tekomi/assistant_responses/:id' do
    let!(:response_record) { create(:tekomi_assistant_response, assistant: assistant) }
    ...
    it 'updates the response if the user is an admin' do
      patch "/api/v1/accounts/#{account.id}/tekomi/assistant_responses/#{response_record.id}",
  ...
  describe 'DELETE /api/v1/accounts/:account_id/tekomi/assistant_responses/:id' do
    let!(:response_record) { create(:tekomi_assistant_response, assistant: assistant) }

    it 'deletes the response' do
      expect do
        delete "/api/v1/accounts/#{account.id}/tekomi/assistant_responses/#{response_record.id}",
               headers: admin.create_new_auth_token,
               as: :json
      end.to change(Tekomi::AssistantResponse, :count).by(-1)
    ...
    context 'with invalid id' do
      it 'returns not found' do
        delete "/api/v1/accounts/#{account.id}/tekomi/assistant_responses/0",
               headers: admin.create_new_auth_token,
               as: :json
