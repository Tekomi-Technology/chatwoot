...
RSpec.describe 'Api::V1::Accounts::Tekomi::Documents', type: :request do
  let(:account) { create(:account, custom_attributes: { plan_name: 'startups' }) }
  let(:admin) { create(:user, account: account, role: :administrator) }
  let(:agent) { create(:user, account: account, role: :agent) }
  let(:assistant) { create(:tekomi_assistant, account: account) }
  let(:assistant2) { create(:tekomi_assistant, account: account) }
  let(:document) { create(:tekomi_document, assistant: assistant, account: account, status: :available) }
  ...
  describe 'GET /api/v1/accounts/:account_id/tekomi/documents' do
    context 'when it is an un-authenticated user' do
      before do
        get "/api/v1/accounts/#{account.id}/tekomi/documents"
      end
    ...
    context 'when it is an agent' do
      context 'when no filters are applied' do
        before do
          create_list(:tekomi_document, 30, assistant: assistant, account: account)
        end

        it 'returns the first page of documents' do
          get "/api/v1/accounts/#{account.id}/tekomi/documents", headers: agent.create_new_auth_token, as: :json
        ...
        it 'returns the generated FAQ count for each document' do
          document = create(:tekomi_document, assistant: assistant, account: account)
          create_list(:tekomi_assistant_response, 2,
      ...
      context 'when filtering by assistant_id' do
        before do
          create_list(:tekomi_document, 3, assistant: assistant, account: account)
          create_list(:tekomi_document, 2, assistant: assistant2, account: account)
        end
        ...
        it 'returns empty array when assistant has no documents' do
          new_assistant = create(:tekomi_assistant, account: account)
          get "/api/v1/accounts/#{account.id}/tekomi/documents",
      ...
      context 'when documents belong to different accounts' do
        ...
        before do
          create_list(:tekomi_document, 3, assistant: assistant, account: account)
          create_list(:tekomi_document, 2, account: other_account)
        end
      ...
      context 'with pagination and assistant filter combined' do
        before do
          create_list(:tekomi_document, 30, assistant: assistant, account: account)
          create_list(:tekomi_document, 10, assistant: assistant2, account: account)
        end
    ...
    context 'when it is an admin' do
      it 'returns the distinct conversation usage count for each document' do
        ...
        other_document = create(:tekomi_document, assistant: assistant, account: account)

        create(:tekomi_agent_session, account: account, assistant: assistant,
        ...
        create(:tekomi_agent_session, account: account, assistant: assistant2,
                                       subject: second_conversation, document_ids: [document.id], credits_consumed: 1.0)
      ...
      it 'excludes handoff sessions from document usage' do
        answered_document = create(:tekomi_document, assistant: assistant, account: account)
        ...
        get "/api/v1/accounts/#{account.id}/tekomi/documents/#{document.id}/drilldown",
            headers: admin.create_new_auth_token, as: :json
      ...
      it 'sorts documents by the number of distinct conversations that used them' do
        most_used_document = create(:tekomi_document, assistant: assistant, account: account)
        less_used_document = create(:tekomi_document, assistant: assistant, account: account)
        unused_document = create(:tekomi_document, assistant: assistant, account: account)
        first_conversation = create(:conversation, account: account)
        second_conversation = create(:conversation, account: account)
      ...
      it 'excludes deleted conversations from usage counts and sorting' do
        used_document = create(:tekomi_document, assistant: assistant, account: account)
        unused_document = create(:tekomi_document, assistant: assistant, account: account)
        deleted_conversation = create(:conversation, account: account)
        deleted_session = create(:tekomi_agent_session, account: account, assistant: assistant,
  ...
  describe 'GET /api/v1/accounts/:account_id/tekomi/documents/:id' do
    context 'when it is an un-authenticated user' do
      before do
        get "/api/v1/accounts/#{account.id}/tekomi/documents/#{document.id}"
      end
  ...
  describe 'GET /api/v1/accounts/:account_id/tekomi/documents/:id/drilldown' do
    ...
    it 'does not return sessions from another document' do
      ...
      get "/api/v1/accounts/#{account.id}/tekomi/documents/#{other_document.id}/drilldown",
          headers: admin.create_new_auth_token, as: :json
  ...
  describe 'POST /api/v1/accounts/:account_id/tekomi/documents' do
    ...
    context 'when it is an un-authenticated user' do
      before do
        post "/api/v1/accounts/#{account.id}/tekomi/documents",
    ...
    context 'when it is an admin' do
      context 'with valid parameters' do
        it 'creates a new document' do
          ...
          end.to change(Tekomi::Document, :count).by(1)
        end
      ...
      context 'with limits exceeded' do
        before do
          create_list(:tekomi_document, 5, assistant: assistant, account: account)

          InstallationConfig.find_or_initialize_by(name: 'TEKOMI_CLOUD_PLAN_LIMITS').update!(value: tekomi_limits.to_json)
          post "/api/v1/accounts/#{account.id}/tekomi/documents",
               params: valid_attributes,
               headers: admin.create_new_auth_token
        end
  ...
  describe 'POST /api/v1/accounts/:account_id/tekomi/documents/:id/sync' do
    before { clear_enqueued_jobs }

    context 'when it is an un-authenticated user' do
      it 'returns unauthorized status' do
        post "/api/v1/accounts/#{account.id}/tekomi/documents/#{document.id}/sync"
        expect(response).to have_http_status(:unauthorized)
      end
    end
    ...
    context 'when it is an admin' do
      it 'queues a sync and returns accepted' do
        freeze_time do
          expect do
            post "/api/v1/accounts/#{account.id}/tekomi/documents/#{document.id}/sync",
                 headers: admin.create_new_auth_token, as: :json
          end.to have_enqueued_job(Tekomi::Documents::PerformSyncJob).with(document).on_queue('low')
      ...
      it 'queues stale syncing documents again' do
        freeze_time do
          document.update!(sync_status: :syncing, last_sync_attempted_at: (Tekomi::Document::SYNC_STALE_TIMEOUT + 1.minute).ago)

          expect do
            post "/api/v1/accounts/#{account.id}/tekomi/documents/#{document.id}/sync",
                 headers: admin.create_new_auth_token, as: :json
          end.to have_enqueued_job(Tekomi::Documents::PerformSyncJob).with(document)
      ...
      it 'rejects PDF documents with an explanatory error' do
        pdf_document = build(:tekomi_document, assistant: assistant, account: account)
        pdf_document.pdf_file.attach(io: StringIO.new('PDF content'), filename: 'test.pdf',
                                     content_type: 'application/pdf')
        pdf_document.save!

        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/documents/#{pdf_document.id}/sync",
               headers: admin.create_new_auth_token, as: :json
        end.not_to have_enqueued_job(Tekomi::Documents::PerformSyncJob)

        expect(response).to have_http_status(:unprocessable_entity)
      end

      it 'rejects documents that are still being processed' do
        in_progress_document = create(:tekomi_document, assistant: assistant, account: account, status: :in_progress)

        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/documents/#{in_progress_document.id}/sync",
               headers: admin.create_new_auth_token, as: :json
        end.not_to have_enqueued_job(Tekomi::Documents::PerformSyncJob)
  ...
  describe 'DELETE /api/v1/accounts/:account_id/tekomi/documents/:id' do
    context 'when it is an un-authenticated user' do
      before do
        delete "/api/v1/accounts/#{account.id}/tekomi/documents/#{document.id}"
      end
    ...
    context 'when it is an agent' do
      let!(:document_to_delete) { create(:tekomi_document, assistant: assistant) }

      it 'deletes the document' do
        delete "/api/v1/accounts/#{account.id}/tekomi/documents/#{document_to_delete.id}",
               headers: agent.create_new_auth_token
    ...
    context 'when it is an admin' do
      ...
      context 'when document does not exist' do
        before do
          delete "/api/v1/accounts/#{account.id}/tekomi/documents/invalid_id",
                 headers: admin.create_new_auth_token
        end
