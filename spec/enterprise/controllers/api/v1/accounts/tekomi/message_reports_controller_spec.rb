...
RSpec.describe 'Api::V1::Accounts::Tekomi::MessageReports', type: :request do
  ...
  describe 'POST /api/v1/accounts/:account_id/tekomi/message_reports' do
    let(:valid_params) do
      {
    ...
    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        post "/api/v1/accounts/#{account.id}/tekomi/message_reports", params: valid_params, as: :json
    ...
    context 'when the installation is not on Chatwoot cloud' do
      ...
      it 'returns not found' do
        post "/api/v1/accounts/#{account.id}/tekomi/message_reports",
      ...
      it 'does not create a report' do
        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/message_reports",
               params: valid_params, headers: agent.create_new_auth_token, as: :json
        end.not_to change(Tekomi::MessageReport, :count)
      end
    end
    ...
    context 'when on Chatwoot cloud' do
      ...
      it 'creates a message report for the reporting agent' do
        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/message_reports",
               params: valid_params, headers: agent.create_new_auth_token, as: :json
        end.to change(Tekomi::MessageReport, :count).by(1)

        report = Tekomi::MessageReport.last
      ...
      it 'rejects messages that were not sent by a Tekomi assistant' do
        non_tekomi_message = create(:message, account: account, conversation: conversation)

        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/message_reports",
               params: valid_params.merge(message_id: non_tekomi_message.id),
               headers: agent.create_new_auth_token, as: :json
        end.not_to change(Tekomi::MessageReport, :count)
