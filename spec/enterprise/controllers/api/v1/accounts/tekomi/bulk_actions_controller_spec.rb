...
RSpec.describe 'Api::V1::Accounts::Tekomi::BulkActions', type: :request do
  ...
  describe 'POST /api/v1/accounts/:account_id/tekomi/bulk_actions' do
      ...
      it 'deletes the documents and returns the deleted count' do
        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/bulk_actions",
               ...
               headers: admin.create_new_auth_token,
               as: :json
        end.to change(Tekomi::Document, :count).by(-2)
    ...
    context 'when syncing documents' do
        ...
        freeze_time do
          expect do
            post "/api/v1/accounts/#{account.id}/tekomi/bulk_actions",
                 ...
                 headers: admin.create_new_auth_token,
                 as: :json
          end.to have_enqueued_job(Tekomi::Documents::PerformSyncJob).on_queue('low').exactly(documents.size).times
      ...
      it 'queues documents that already have a sync in progress' do
        syncing_document = create(:tekomi_document, assistant: assistant, account: account, status: :available)
        ...
        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/bulk_actions",
               ...
               headers: admin.create_new_auth_token,
               as: :json
        end.to have_enqueued_job(Tekomi::Documents::PerformSyncJob).with(syncing_document).on_queue('low')
      ...
      it 'queues stale syncing documents again' do
        ...
        freeze_time do
          syncing_document.update!(
            sync_status: :syncing,
            last_sync_attempted_at: (Tekomi::Document::SYNC_STALE_TIMEOUT + 1.minute).ago
          )
          ...
          expect do
            post "/api/v1/accounts/#{account.id}/tekomi/bulk_actions",
                 ...
                 headers: admin.create_new_auth_token,
                 as: :json
          end.to have_enqueued_job(Tekomi::Documents::PerformSyncJob).with(syncing_document)
