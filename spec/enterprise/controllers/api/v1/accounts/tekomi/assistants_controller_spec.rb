...
RSpec.describe 'Api::V1::Accounts::Tekomi::Assistants', type: :request do
  ...
  describe 'GET /api/v1/accounts/{account.id}/tekomi/assistants' do
    context 'when it is an un-authenticated user' do
      it 'does not fetch assistants' do
        get "/api/v1/accounts/#{account.id}/tekomi/assistants",
            as: :json
    ...
    context 'when it is an agent' do
      it 'fetches assistants for the account' do
        create_list(:tekomi_assistant, 3, account: account)
        get "/api/v1/accounts/#{account.id}/tekomi/assistants",
  ...
  describe 'GET /api/v1/accounts/{account.id}/tekomi/assistants/{id}' do
    let(:assistant) { create(:tekomi_assistant, account: account) }

    context 'when it is an un-authenticated user' do
      it 'does not fetch the assistant' do
        get "/api/v1/accounts/#{account.id}/tekomi/assistants/#{assistant.id}",
            as: :json
        expect(response).to have_http_status(:unauthorized)
      end
    end
  ...
  describe 'POST /api/v1/accounts/{account.id}/tekomi/assistants' do
    ...
    context 'when it is an un-authenticated user' do
      it 'does not create an assistant' do
        post "/api/v1/accounts/#{account.id}/tekomi/assistants",
    ...
    context 'when it is an admin' do
      it 'creates a new assistant' do
        expect do
          post "/api/v1/accounts/#{account.id}/tekomi/assistants",
               ...
               as: :json
        end.to change(Tekomi::Assistant, :count).by(1)
      ...
      it 'stores evaluated mode on the assistant when tekomi_tasks is enabled' do
        ...
        assistant = Tekomi::Assistant.find(json_response[:id])
        expect(assistant.config['auto_resolve_mode']).to eq('evaluated')
        expect(account.reload.settings).not_to have_key('tekomi_auto_resolve_mode')
      end

      it 'stores legacy mode on the assistant when tekomi_tasks is disabled' do
        account.disable_features!('tekomi_tasks')
  ...
  describe 'PATCH /api/v1/accounts/{account.id}/tekomi/assistants/{id}' do
    let(:assistant) { create(:tekomi_assistant, account: account) }
    ...
    context 'when it is an un-authenticated user' do
      it 'does not update the assistant' do
        patch "/api/v1/accounts/#{account.id}/tekomi/assistants/#{assistant.id}",
    ...
    context 'when it is an admin' do
      ...
      it 'keeps inactivity timer settings behind Tekomi V2' do
      ...
      it 'updates inactive conversation settings for Tekomi v2' do
  ...
  describe 'DELETE /api/v1/accounts/{account.id}/tekomi/assistants/{id}' do
    let!(:assistant) { create(:tekomi_assistant, account: account) }

    context 'when it is an un-authenticated user' do
      it 'does not delete the assistant' do
        delete "/api/v1/accounts/#{account.id}/tekomi/assistants/#{assistant.id}",
               as: :json
        expect(response).to have_http_status(:unauthorized)
      end
    end
  ...
  describe 'GET /api/v1/accounts/{account.id}/tekomi/assistants/{id}/faq_stats' do
    let(:assistant) { create(:tekomi_assistant, account: account) }

    it 'returns approved FAQ, open suggestion, document counts and coverage' do
      create_list(:tekomi_assistant_response, 3, assistant: assistant, account: account, status: :approved)
      assistant.faq_suggestions.create!(question: 'How do I enable the feature?', answer: 'Turn it on in settings.')
      create_list(:tekomi_document, 2, assistant: assistant, account: account)

      get "/api/v1/accounts/#{account.id}/tekomi/assistants/#{assistant.id}/faq_stats",
          headers: admin.create_new_auth_token,
          as: :json
    ...
    it 'counts only suggestions backed by conversations the agent can access' do
      ...
      create(:tekomi_assistant_response, assistant: assistant, account: account, status: :approved)
  ...
  describe 'GET /api/v1/accounts/{account.id}/tekomi/assistants/{id}/summary' do
    let(:assistant) { create(:tekomi_assistant, account: account) }
    ...
    let(:summary_service) { instance_double(Tekomi::OverviewSummaryService) }
    ...
    def get_summary(user)
      get "/api/v1/accounts/#{account.id}/tekomi/assistants/#{assistant.id}/summary",
    ...
    before do
      ...
      allow(Tekomi::OverviewSummaryService).to receive(:new).and_return(summary_service)
    end
    ...
    it 'caches the summary per viewer so one user never receives another user\'s greeting' do
      ...
      expect(Tekomi::AssistantStatsBuilder).not_to receive(:new)
      ...
      expect(response).to have_http_status(:success)
      expect(Tekomi::OverviewSummaryService).to have_received(:new).twice
      expect(Tekomi::OverviewSummaryService).to have_received(:new).with(hash_including(stats: summary_stats)).twice
    end
  ...
  describe 'POST /api/v1/accounts/{account.id}/tekomi/assistants/{id}/playground' do
    let(:assistant) { create(:tekomi_assistant, account: account) }
    let(:valid_params) do
      {
    ...
    let(:chat_service) { instance_double(Tekomi::Llm::AssistantChatService) }
    let(:agent_runner_service) { instance_double(Tekomi::Assistant::AgentRunnerService) }

    context 'when it is an un-authenticated user' do
      it 'returns unauthorized' do
        post "/api/v1/accounts/#{account.id}/tekomi/assistants/#{assistant.id}/playground",
             params: valid_params,
             as: :json
    ...
    context 'when tekomi v2 is disabled' do
      it 'generates a response with the legacy assistant chat service' do
        allow(Tekomi::Llm::AssistantChatService).to receive(:new).with(
          assistant: assistant,
          source: 'playground'
    ...
    context 'when tekomi v2 is enabled' do
      before do
        account.enable_features('tekomi_integration_v2')
      end

      it 'generates a response with the agent runner service' do
        allow(Tekomi::Assistant::AgentRunnerService).to receive(:new).with(
          assistant: assistant,
          source: 'playground'
        ...
        expect(Tekomi::Llm::AssistantChatService).not_to receive(:new)
