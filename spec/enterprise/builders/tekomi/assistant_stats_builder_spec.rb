...
RSpec.describe Tekomi::AssistantStatsBuilder do
  let(:account) { create(:account) }
  let(:assistant) { create(:tekomi_assistant, account: account) }
  let(:inbox) { create(:inbox, account: account) }

  before { create(:tekomi_inbox, tekomi_assistant: assistant, inbox: inbox) }
  ...
  describe '#metrics' do
    ...
    it 'excludes resolution events that fall outside the current window' do
      create(:reporting_event, account: account, conversation: current_convo_a,
                               name: 'conversation_tekomi_inference_resolved', created_at: 60.days.ago)
  ...
  describe '#metrics reopen_rate' do
    ...
    it 'counts a reopen that happened after the tekomi resolve' do
    ...
    it 'ignores a human resolve/reopen that happened before the tekomi resolve' do
      # Earlier resolve/reopen cycle, then Tekomi resolves later in the same window.
      create(:reporting_event, account: account, inbox: inbox, conversation: conversation,
    ...
    it 'counts both inference and time-based bot resolves in the denominator' do
      ...
      create(:reporting_event, account: account, inbox: inbox, conversation: conversation,
                               name: 'conversation_tekomi_inference_resolved', event_start_time: 6.days.ago, event_end_time: 6.days.ago)
      create(:reporting_event, account: account, inbox: inbox, conversation: conversation,
                               name: 'conversation_opened', value: 120, event_start_time: 6.days.ago, event_end_time: 4.days.ago)
    ...
    it 'derives the cohort from handled conversations, not current inbox membership' do
      ...
      TekomiInbox.where(tekomi_assistant: assistant).delete_all
  ...
  describe '#faq_stats' do
    ...
    it 'reports zero coverage when there are no FAQs or suggestions' do
      Tekomi::AssistantResponse.where(assistant: assistant).delete_all
      Tekomi::FaqSuggestion.where(assistant: assistant).delete_all
