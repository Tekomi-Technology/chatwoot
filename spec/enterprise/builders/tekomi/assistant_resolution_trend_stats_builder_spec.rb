...
RSpec.describe Tekomi::AssistantResolutionTrendStatsBuilder do
  ...
  context 'with no outcomes' do
    it 'returns zeroed weekly buckets for the complete reporting window' do
      expect(metrics).to eq(
        granularity: :week,
        buckets: [
          { starts_on: Date.new(2025, 6, 1), ends_on: Date.new(2025, 6, 7), conversations_handled: 0, resolved_by_tekomi: 0 },
          { starts_on: Date.new(2025, 6, 8), ends_on: Date.new(2025, 6, 14), conversations_handled: 0, resolved_by_tekomi: 0 },
          { starts_on: Date.new(2025, 6, 15), ends_on: Date.new(2025, 6, 21), conversations_handled: 0, resolved_by_tekomi: 0 },
          { starts_on: Date.new(2025, 6, 22), ends_on: Date.new(2025, 6, 28), conversations_handled: 0, resolved_by_tekomi: 0 },
          { starts_on: Date.new(2025, 6, 29), ends_on: Date.new(2025, 6, 30), conversations_handled: 0, resolved_by_tekomi: 0 }
        ]
      )
    end
  end
  ...
  context 'when the reporting window spans 15 days or less' do
    ...
    it 'returns zeroed daily buckets' do
      ...
      expect(metrics[:buckets]).to all(include(conversations_handled: 0, resolved_by_tekomi: 0))
    end
  ...
  context 'with outcomes across the reporting window' do
    before do
      ...
      create(
        ...
        first_tekomi_reply_at: first_week_start + 1.minute,
      ...
      create(
        ...
        first_tekomi_reply_at: fourth_week_start + 1.minute,
      ...
      create(
        ...
        first_tekomi_reply_at: final_week_start + 1.minute,
      ...
      create(
        ...
        first_tekomi_reply_at: outside_window_start + 1.minute,
        resolved_at: outside_window_start + 1.hour
      )

      other_assistant = create(:tekomi_assistant, account: account)
      ...
      create(
        ...
        first_tekomi_reply_at: other_assistant_start + 1.minute,
        resolved_at: other_assistant_start + 1.hour
      )
    end

    it 'counts handled and Tekomi-resolved outcomes in their demand week' do
      bucket_counts = metrics[:buckets].map { |bucket| bucket.values_at(:conversations_handled, :resolved_by_tekomi) }
  ...
  it 'anchors calendar buckets to the viewer timezone' do
    ...
    create(
      ...
      first_tekomi_reply_at: started_at + 1.minute,
