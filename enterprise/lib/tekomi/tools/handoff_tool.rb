class Tekomi::Tools::HandoffTool < Tekomi::Tools::BasePublicTool
  ...
  def trigger_handoff(tool_context, conversation, reason, reason_category)
    return trigger_legacy_handoff(tool_context, conversation, reason, reason_category) unless tekomi_v2_enabled?
    ...
    tool_context.state[:tekomi_v2_handoff_tool_completed] = true
    ...
    # Send out of office message if applicable (since template messages were suppressed while Tekomi was handling)
  ...
  def emit_tool_handoff_event(conversation, reason_category)
    Tekomi::ConversationEvents.handed_off(conversation: conversation, assistant: @assistant,
                                          source: Tekomi::ConversationEvents::Sources::TOOL,
