class Tekomi::Tools::ResolveConversationTool < Tekomi::Tools::BasePublicTool
  ...
  def perform(tool_context, reason:)
    ...
    conversation.with_tekomi_activity_context(reason: reason, reason_type: :tool) { conversation.resolved! }
    Tekomi::ConversationEvents.resolved(conversation: conversation, assistant: @assistant,
                                        source: Tekomi::ConversationEvents::Sources::TOOL, at: Time.current)
