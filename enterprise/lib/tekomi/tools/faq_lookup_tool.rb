class Tekomi::Tools::FaqLookupTool < Tekomi::Tools::BasePublicTool
  ...
  def record_retrieved_sources(tool_context, responses)
    ...
    document_ids = Array(responses_by_type['Tekomi::Document']).map(&:documentable_id)
  ...
  def citation_index(tool_context, response)
    citation_document_ids = tool_context.state[Tekomi::Assistant::CITATION_SOURCES_STATE_KEY] ||= {}
