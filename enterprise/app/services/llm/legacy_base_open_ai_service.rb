...
# - Tekomi::Llm::PdfProcessingService (files.upload for assistants)
# - Tekomi::Llm::PaginatedFaqGeneratorService (uses file_id from uploaded files)
#
...
class Llm::LegacyBaseOpenAiService
  ...
  def initialize
    @client = OpenAI::Client.new(
      access_token: InstallationConfig.find_by!(name: 'TEKOMI_OPEN_AI_API_KEY').value,
      uri_base: uri_base,
  ...
  def uri_base
    endpoint = InstallationConfig.find_by(name: 'TEKOMI_OPEN_AI_ENDPOINT')&.value
    endpoint.presence || 'https://api.openai.com/'
  end

  def setup_model
    config_value = InstallationConfig.find_by(name: 'TEKOMI_OPEN_AI_MODEL')&.value
