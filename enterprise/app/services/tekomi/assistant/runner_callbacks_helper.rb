module Tekomi::Assistant::RunnerCallbacksHelper
  private
  ...
  def register_trace_input_callback(runner)
    runner.on_agent_thinking do |_agent_name, _input, context_wrapper|
      ...
      trace_input = context_wrapper.context[:tekomi_v2_trace_current_input]
  ...
  def add_agent_thinking_callback(runner)
    runner.on_agent_thinking do |*args|
      ...
      Rails.logger.warn "[Tekomi] Callback error for agent_thinking: #{e.message}"
    end
  end
  ...
  def add_tool_start_callback(runner)
    runner.on_tool_start do |*args|
      ...
      Rails.logger.warn "[Tekomi] Callback error for tool_start: #{e.message}"
    end
  end
  ...
  def add_tool_complete_callback(runner)
    runner.on_tool_complete do |*args|
      ...
      Rails.logger.warn "[Tekomi] Callback error for tool_complete: #{e.message}"
    end
  end
  ...
  def add_agent_handoff_callback(runner)
    runner.on_agent_handoff do |*args|
      ...
      Rails.logger.warn "[Tekomi] Callback error for agent_handoff: #{e.message}"
    end
  end
end
