module Tekomi::Assistant::AgentRunResponse
  private

  def process_agent_result(run_result)
    Rails.logger.info "[Tekomi V2] Agent result: #{run_result.inspect}"
    ...
    response_parts = Tekomi::Assistant::ResponseParts.from_response(structured_response)
    ...
    structured_response['handoff_tool_called'] = run_result.context&.dig(:tekomi_v2_handoff_tool_called) || false
    structured_response
  end
  ...
  def rewrite_oversized_response(run_result)
    ...
    raise 'Tekomi citation links exceed the channel limit' unless response_text_limit.positive?
  ...
  def response_rewriter
    @response_rewriter ||= Tekomi::Assistant::ResponseRewriter.new(
      assistant: @assistant,
      attribute_provider: Tekomi::Assistant::InstrumentationAttributeProvider.new(self)
    )
  end
  ...
  def record_turn_start(run_result)
    ...
    run_result.context[:tekomi_v2_turn_start_index] = turn_start_index if turn_start_index
  end
  ...
  def message_length_limit
    @message_length_limit ||= Tekomi::MessageLengthLimit.for(@conversation)
  end
