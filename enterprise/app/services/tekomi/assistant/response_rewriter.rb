...
class Tekomi::Assistant::ResponseRewriter
  include Integrations::LlmInstrumentationConstants

  AGENT_NAME = 'tekomi_response_rewriter'.freeze
  ...
  def rewritten_response_with_original_citations(rewritten_model_output, original_response_parts)
    ...
    rewritten_response_parts = Tekomi::Assistant::ResponseParts.from_response(rewritten_model_output)
    original_parts = original_response_parts.to_a
    rewritten_parts = rewritten_response_parts.to_a
    raise 'Tekomi response rewrite changed the number of response parts' unless rewritten_parts.size == original_parts.size
    ...
    if @assistant.citations_enabled? && rewritten_citation_indexes != original_citation_indexes
      raise 'Tekomi response rewrite changed the response part citation order'
    end
  ...
  def install_instrumentation(runner)
    ...
    Agents::Instrumentation.install(
      ...
      trace_name: 'llm.tekomi_v2.rewrite',
      span_attributes: {
        ATTR_LANGFUSE_TAGS => %w[tekomi_v2 channel_limit_rewrite].to_json,
