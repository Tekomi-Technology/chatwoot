...
class Tekomi::Assistant::AgentRunnerService
  include Tekomi::Assistant::RunnerCallbacksHelper
  include Tekomi::Assistant::AgentRunResponse
  include Tekomi::Assistant::RunnerInstrumentationHelper
  include Tekomi::Assistant::TracePayloadHelper
  include Tekomi::Assistant::RunnerStateHelper
  ...
  def generate_response(message_history: [])
    ...
    Rails.logger.error "[Tekomi V2] AgentRunnerService error: #{e.message}"
  ...
  def extract_last_user_message(message_history)
    ...
    text, attachments = Tekomi::OpenAiMessageBuilderService.extract_text_and_attachments(content)
  ...
  def extract_text_from_content(content)
    ...
    if content.is_a?(Hash)
      response_text = Tekomi::Assistant::ResponseParts.from_response(content).plain_text
  ...
  def available_in_reply_suggestion?(tool)
    return true if tool.is_a?(Tekomi::Tools::FaqLookupTool)

    tool.is_a?(Tekomi::Tools::HttpTool) && tool.available_in_reply_suggestion?
  end
