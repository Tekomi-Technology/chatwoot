class Tekomi::Assistant::SessionCaptureService
  SCENARIO_AGENT_REGEX = /\A#{Tekomi::Scenario::HANDOFF_KEY_PREFIX}_(\d+)_/
  ...
  def capture
    ...
    Rails.logger.error("[TEKOMI][SessionCaptureService] Capture failed for conversation=#{@conversation.display_id}: #{e.message}")
  end
  ...
  def cited_document_ids
    ...
    citation_document_ids = (context.dig(:state, Tekomi::Assistant::CITATION_SOURCES_STATE_KEY) || {}).transform_keys(&:to_i)
    visible_citation_indexes = @assistant.customer_visible_citation_urls(citation_document_ids).keys
    stored_response_parts = result_message.additional_attributes.to_h[Tekomi::Assistant::ResponseParts::MESSAGE_ATTRIBUTE_KEY]
    response_parts = Tekomi::Assistant::ResponseParts.new(stored_response_parts)
  ...
  def current_turn_history
    ...
    turn_start_index = context[:tekomi_v2_turn_start_index] || history.rindex { |message| message[:role].to_s == 'user' } || 0
