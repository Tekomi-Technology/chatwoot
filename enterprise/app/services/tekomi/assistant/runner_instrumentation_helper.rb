module Tekomi::Assistant::RunnerInstrumentationHelper
  ...
  def dynamic_trace_attributes(context_wrapper)
    ...
    trace_input = context_wrapper&.context&.dig(:tekomi_v2_trace_input)
  ...
  def add_usage_metadata_callback(runner)
    handoff_tool_name = Tekomi::Tools::HandoffTool.new(@assistant).name
  ...
  def track_handoff_usage(tool_name, handoff_tool_name, context_wrapper)
    ...
    context_wrapper.context[:tekomi_v2_handoff_tool_called] = true
    @handoff_tool_called = true

    return unless context_wrapper.context.dig(:state, :tekomi_v2_handoff_tool_completed)
