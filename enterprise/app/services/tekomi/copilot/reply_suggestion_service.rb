class Tekomi::Copilot::ReplySuggestionService
  include Tekomi::Copilot::ConversationAccess
  include Integrations::LlmInstrumentationConstants

  class GenerationError < StandardError; end

  TRACE_NAME = 'llm.tekomi.copilot'.freeze
  ...
  def conversation_history(conversation)
    history = Tekomi::Conversation::MessageHistoryBuilderService.new(conversation: conversation).perform
    history.pop while history.last&.dig(:content) == Tekomi::Conversation::MessageHistoryBuilderService::RESOLUTION_MARKER
    history
  end

  def generate_reply(conversation)
    runner = Tekomi::Assistant::AgentRunnerService.new(
      assistant: @assistant,
      conversation: conversation,
      source: Tekomi::Assistant::AgentRunnerService::REPLY_SUGGESTION_SOURCE
    )
  ...
  def persist_response(response, runner, conversation, target_message)
    response_parts = Tekomi::Assistant::ResponseParts.from_response(response)
  ...
  def discarded_response
    {
      'response' => I18n.t('tekomi.copilot.reply_suggestion_discarded', locale: reply_locale),
      'discarded' => true
    }
  end
  ...
  def failure_response
    {
      'response' => I18n.t('tekomi.copilot.reply_suggestion_failed', locale: reply_locale),
      'discarded' => false
    }
  end
end
