class Tekomi::Llm::AssistantActionClassifierService < Llm::BaseAiService
  ...
  def classify(message_history:, assistant_response:)
    ...
    response = instrument_llm_call(instrumentation_params(user_prompt)) do
      chat(model: @model, temperature: @temperature)
        .with_schema(Tekomi::AssistantActionSchema)
    ...
    Rails.logger.warn(
      "[TEKOMI][AssistantActionClassifier] Failed for conversation #{@conversation.display_id}: #{e.class.name}: #{e.message}"
    )
  ...
  def normalize_response(parsed, raw_content)
    ...
    return invalid_response(raw_content) unless Tekomi::AssistantActionSchema::ACTIONS.include?(action)
  ...
  def instrumentation_params(user_prompt)
    {
      span_name: 'llm.tekomi.assistant_action_classifier',
      model: @model,
