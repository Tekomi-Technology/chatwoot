class Tekomi::Conversation::MessageHistoryBuilderService
  ...
  def prepare_multimodal_message_content(message)
    if message.outgoing? && message.sender_type == 'Tekomi::Assistant' && !message.deleted
      message_attributes = message.additional_attributes.to_h
      response_parts_attribute = Tekomi::Assistant::ResponseParts::MESSAGE_ATTRIBUTE_KEY
      if message_attributes.key?(response_parts_attribute)
        return Tekomi::Assistant::ResponseParts.new(message_attributes[response_parts_attribute]).plain_text.presence
      end
    end

    Tekomi::OpenAiMessageBuilderService.new(message: message).generate_content
  end
end
