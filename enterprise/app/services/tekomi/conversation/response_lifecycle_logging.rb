module Tekomi::Conversation::ResponseLifecycleLogging
  private
  ...
  def log_non_pending
    ...
    Rails.logger.info(
      "[TEKOMI][ResponseLifecycle] event=job_skipped reason=conversation_not_pending conversation_id=#{@conversation.id} " \
  ...
  def log_pre_generation_discard
    Rails.logger.info(
      '[TEKOMI][ResponseLifecycle] event=response_discarded reason=newer_customer_message_before_generation ' \
