class Tekomi::ConversationOutcomeTracker
    ...
    safely_track(:reopen, at: at) do
      ...
      ApplicationRecord.transaction do
        ...
        ConversationOutcome.create!(
          account: account,
          assistant: conversation.inbox.tekomi_assistant || predecessor.assistant,
  ...
  def snapshot_message_facts(episode, through:)
    ...
    replies = messages.where(sender_type: 'Tekomi::Assistant', message_type: :outgoing, private: false)

    episode.tekomi_reply_count = replies.count
    episode.first_tekomi_reply_at = replies.minimum(:created_at)
    episode.last_tekomi_reply_at = replies.maximum(:created_at)
  end
  ...
  def safely_track(action, at:)
    ...
    Rails.logger.error(
      "[TEKOMI][ConversationOutcomeTracker] Failed to record #{action} for conversation=#{conversation.display_id} " \
