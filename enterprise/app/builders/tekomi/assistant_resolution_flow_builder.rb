...
class Tekomi::AssistantResolutionFlowBuilder
  include Tekomi::AssistantOutcomeClassification
  ...
  def initialize(assistant, range = Tekomi::AssistantStatsWindow::DEFAULT_RANGE, timezone_offset = nil)
    @assistant = assistant
  ...
  def base_links(counts)
    [
      link(:conversations_handled, :resolved_by_tekomi, counts[:resolved_by_tekomi]),
      ...
      link(:resolved_by_tekomi, :reopened_within_7_days, counts[:reopened_within_7_days]),
      link(:resolved_by_tekomi, :stayed_closed, counts[:stayed_closed])
    ]
  end
