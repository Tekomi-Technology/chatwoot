# Lists the conversations where Tekomi used one durable knowledge record.
...
class Tekomi::ConversationUsageBuilder
  DEFAULT_PAGE = 1
  DEFAULT_PER_PAGE = 25
  MAX_PER_PAGE = 100
  ...
  class << self
    ...
    def order_documents_by_conversation_count(documents, account_id:, assistant_id: nil)
      ...
      documents_table = Tekomi::Document.arel_table
      usage_counts = Arel::Table.new('tekomi_document_usage')
      ...
      documents
        .joins(
          <<~SQL.squish
            LEFT JOIN (#{usage_counts_sql}) AS tekomi_document_usage
              ON tekomi_document_usage.resource_id = tekomi_documents.id::text
              AND tekomi_document_usage.assistant_id = tekomi_documents.assistant_id
          SQL
        )
    ...
    def usage_count_rows(resources, usage_column)
      resources.each_slice(DEFAULT_PER_PAGE).flat_map do |batch|
        Tekomi::AgentSession.connection.select_rows(usage_count_sql(batch, usage_column))
      end
    end
    ...
    def indexed_usage_sessions(resource, usage_column)
      Tekomi::AgentSession.session_assistant
                          ...
                          .where(Tekomi::AgentSession.arel_table[usage_column].contains([resource.id]))
    ...
    def document_usage_counts(account_id, assistant_id)
      sessions_table = Tekomi::AgentSession.arel_table
      knowledge_usage = Arel::Table.new('knowledge_usage')
      sessions = Tekomi::AgentSession.session_assistant
                                     .with_delivered_answer
