# Lists the underlying records behind a single Tekomi assistant stat card, so a
# viewer can drill from an aggregate (e.g. "auto-resolution 42%") into the exact
# conversations that produced it.
#
# The window is resolved by Tekomi::AssistantStatsWindow from the same `range`
...
class Tekomi::AssistantDrilldownBuilder
  ASSISTANT_SENDER_TYPE = 'Tekomi::Assistant'.freeze
  RESOLVED_EVENT_NAMES = Tekomi::AssistantStatsBuilder::RESOLVED_EVENT_NAMES
  HANDOFF_EVENT_NAMES = Tekomi::AssistantStatsBuilder::HANDOFF_EVENT_NAMES
  ...
  # Tekomi resolves in the window, excluding bot-resolved rows whose conversation
  ...
  def resolved_events
    ...
    account.reporting_events
           ...
           .where("NOT (name = ? AND conversation_id IN (#{handoff_ids.to_sql}))",
                  Tekomi::AssistantStatsBuilder::BOT_RESOLVED_EVENT_NAME)
  end

  # Auto-resolved conversations that reopened at/after their Tekomi resolve,
