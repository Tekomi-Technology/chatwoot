# Resolves the current and previous comparison windows for Tekomi assistant
...
#
# Shared by Tekomi::AssistantStatsBuilder (which needs both windows) and
# Tekomi::AssistantDrilldownBuilder (which drills into the current window), so a
# drilldown always covers exactly the rows its stat card counted.
class Tekomi::AssistantStatsWindow
