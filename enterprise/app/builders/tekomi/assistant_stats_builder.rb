# Computes per-assistant overview metrics for the Tekomi Overview page.
...
class Tekomi::AssistantStatsBuilder
  RESOLVED_EVENT_NAMES = %w[conversation_tekomi_inference_resolved conversation_bot_resolved].freeze
  HANDOFF_EVENT_NAMES = %w[conversation_tekomi_inference_handoff conversation_bot_handoff].freeze
  ...
  def initialize(assistant, range = Tekomi::AssistantStatsWindow::DEFAULT_RANGE, timezone_offset = nil, suggestions_scope: nil)
    @assistant = assistant
    @account = assistant.account
    @window = Tekomi::AssistantStatsWindow.new(range, timezone_offset)
  ...
  def faq_stats
    approved, suggestions, documents = Tekomi::AssistantResponse.by_assistant(assistant.id).reorder(nil).pick(
      Arel.sql("COUNT(*) FILTER (WHERE status = #{Tekomi::AssistantResponse.statuses['approved']})"),
      Arel.sql("(#{open_suggestion_count_sql})"),
      Arel.sql("(SELECT COUNT(*) FROM tekomi_documents WHERE assistant_id = #{assistant.id.to_i})")
    )
  ...
  def handled_scope(range)
    account.messages.where(sender_type: 'Tekomi::Assistant', sender_id: assistant.id, created_at: range)
  end
  ...
  # Of the conversations Tekomi auto-resolved, the share reopened afterwards. The cohort is
  ...
  def reopen_rate(range, resolved_count)
    ...
    # Tekomi resolves and keep only reopens at/after one of them, so a human resolve/reopen earlier
    # in the same window isn't mistaken for a reopen-after-Tekomi-resolve. (Comparing the reopen's
