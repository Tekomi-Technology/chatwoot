# Shared query-time classifications for Tekomi conversation outcome reporting.
module Tekomi::AssistantOutcomeClassification
  DURABLE_RESOLUTION_WINDOW = 7.days
  USAGE_LIMIT_REASON = 'usage_limit'.freeze

  private

  # A usage-limit handoff is blocked demand only when Tekomi never replied, so
  # it does not make the episode involved. If Tekomi replied before the quota
  ...
  # means Tekomi participated.
  def involved(table)
    table[:first_tekomi_reply_at].not_eq(nil).or(
  ...
  def autonomous(table)
    table[:resolved_at].not_eq(nil)
                       .and(table[:first_tekomi_reply_at].not_eq(nil))
  ...
  def handoff(table)
    # Intentionally includes usage-limit transfers when a prior Tekomi reply
