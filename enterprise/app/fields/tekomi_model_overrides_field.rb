...
class TekomiModelOverridesField < Administrate::Field::Base
  def feature_rows
  ...
  def selected_override(feature_key)
    resource.tekomi_models&.[](feature_key).presence
  end

  def default_model_id(feature_key)
    return Llm::FeatureRouter::TEKOMI_V2_ASSISTANT_MODEL if feature_key == 'assistant' && resource.feature_enabled?('tekomi_integration_v2')
  ...
  def default_option_label(feature_key)
    ...
    I18n.t('super_admin.tekomi_model_overrides.form.use_default', model: model_label(model_id), model_id: model_id)
  end
  ...
  def internal_default_option_label(feature_key)
    ...
    I18n.t(
      "super_admin.tekomi_model_overrides.form.#{translation_key}",
      model: model_label(route[:model]),
      model_id: route[:model]
    )
  end
  ...
  def feature_name(feature_key)
    I18n.t("super_admin.tekomi_model_overrides.features.#{feature_key}", default: feature_key.humanize)
  end

  def source_label(source)
    I18n.t("super_admin.tekomi_model_overrides.sources.#{source}")
  end
end
