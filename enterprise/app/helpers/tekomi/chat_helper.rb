module Tekomi::ChatHelper
  include Integrations::LlmInstrumentation
  include Tekomi::ChatResponseHelper
  include Tekomi::ChatGenerationRecorder
  ...
  def request_chat_completion
    ...
    with_agent_session do
      ...
      text, attachments = Tekomi::OpenAiMessageBuilderService.extract_text_and_attachments(last_content)
  ...
  def add_messages_to_chat(chat)
    conversation_messages[0...-1].each do |msg|
      text, attachments = Tekomi::OpenAiMessageBuilderService.extract_text_and_attachments(msg[:content])
  ...
  def instrumentation_params(chat = nil)
    {
      span_name: "llm.tekomi.#{feature_name}",
