class Tekomi::InboxPendingConversationsResolutionJob < ApplicationJob
  TEKOMI_INFERENCE_RESOLVE_ACTIVITY_REASON = 'no outstanding questions'.freeze
  TEKOMI_INFERENCE_HANDOFF_ACTIVITY_REASON = 'pending clarification from customer'.freeze
  queue_as :low

  def perform(inbox)
    @tekomi_assistant = inbox.tekomi_assistant
    return if tekomi_assistant.blank? || tekomi_assistant.inactive_conversation_resolution_disabled?

    @inactivity_cutoff_time = Time.current - tekomi_assistant.inactivity_threshold_minutes.minutes
  ...
  attr_reader :tekomi_assistant, :inactivity_cutoff_time

  def evaluate_conversation_completion?(account)
    account.feature_enabled?('tekomi_tasks') && tekomi_assistant.evaluate_inactive_conversations_before_resolving?
  end

  def perform_time_based(inbox)
    Current.executed_by = tekomi_assistant
  ...
  def evaluate_conversation(conversation, inbox)
    Tekomi::ConversationCompletionService.new(
  ...
  def resolve_time_based_conversation(conversation, inbox)
    ...
    Tekomi::ConversationEvents.resolved(
      conversation: conversation,
      assistant: tekomi_assistant,
      source: Tekomi::ConversationEvents::Sources::TIME_BASED,
      at: Time.current
    )
  rescue ActiveRecord::RecordNotFound
    nil
  ...
  def resolve_conversation(conversation, inbox, reason)
    resolved = with_inference_activity_context(conversation, TEKOMI_INFERENCE_RESOLVE_ACTIVITY_REASON) do
  ...
  def record_inference_resolution(conversation)
    Tekomi::ConversationEvents.resolved(
      conversation: conversation,
      assistant: tekomi_assistant,
      source: Tekomi::ConversationEvents::Sources::INFERENCE,
      at: Time.current
    )
  end

  def handoff_conversation(conversation, reason)
    handed_off = with_inference_activity_context(conversation, TEKOMI_INFERENCE_HANDOFF_ACTIVITY_REASON) do
      perform_locked_transition(conversation) do
  ...
  def with_inference_activity_context(conversation, reason, &)
    conversation.with_tekomi_activity_context(reason: reason, reason_type: :inference, &)
  end
  ...
  def create_resolution_message(conversation, inbox)
    return unless tekomi_assistant.send_inactivity_resolution_message?

    I18n.with_locale(inbox.account.locale) do
      resolution_message = tekomi_assistant.config['resolution_message']
      conversation.messages.create!(
        message_type: :outgoing,
        account_id: conversation.account_id,
        inbox_id: conversation.inbox_id,
  ...
  def create_handoff_message(conversation)
    handoff_message = tekomi_assistant.config['handoff_message']
