class Tekomi::Conversation::ResponseBuilderJob < ApplicationJob
  include Tekomi::Conversation::V1ActionClassifier
  include Tekomi::Conversation::V1FalsePromiseHandler
  include Tekomi::Conversation::V2LifecycleEvents
  include Tekomi::Conversation::MessageBuilder
  include Tekomi::Conversation::ResponseLifecycleLogging
  ...
  def perform(conversation, assistant, responding_to_message_id = nil)
    ...
    @assistant = assistant
    @responding_to_message_id = responding_to_message_id if tekomi_v2_enabled?

    return log_non_pending unless conversation_pending?

    Current.executed_by = @assistant

    return generate_and_process_response unless tekomi_v2_enabled?
  ...
  def generate_and_process_response
    ...
    @response = Tekomi::Llm::AssistantChatService.new(assistant: @assistant, conversation: @conversation).generate_response(
  ...
  def generate_response_with_v2
    ...
    message_history = Tekomi::Conversation::MessageHistoryBuilderService.new(conversation: @conversation).perform
  ...
  def v2_runner_service
    ...
    Tekomi::Assistant::AgentRunnerService.new(**runner_args)
  end
  ...
  def process_v1_handoff_request
    ...
    process_v1_handoff
    record_v2_failure_handoff(source: Tekomi::ConversationEvents::Sources::GENERATION_FAILURE) if v2_generation_errored?
  end

  def process_standard_response
    message = nil
    ActiveRecord::Base.transaction do
      next if tekomi_v2_enabled? && newer_customer_message_arrived?

      message = create_messages
      Rails.logger.info("[TEKOMI][ResponseBuilderJob] Incrementing response usage for #{account.id}")
      account.increment_response_usage
    end
    return unless message

    capture_assistant_session(result_message: message, credits_consumed: 1.0)
    record_v2_response_completed(message) if tekomi_v2_enabled?
  end

  def process_v2_handoff_response
    # Tekomi V1 infers completion from status. Tekomi V2 uses the completion
    # marker set inside the locked handoff.
    if tekomi_v2_enabled?
      ...
      record_v2_failure_handoff(source: Tekomi::ConversationEvents::Sources::TOOL) unless v2_handoff_tool_completed?
    else
  ...
  def process_v1_handoff
    I18n.with_locale(@assistant.account.locale) do
      Rails.logger.info(
        "[TEKOMI][ResponseBuilderJob] V1 handoff requested for account=#{account.id} conversation=#{@conversation.display_id} " \
  ...
  def create_handoff_message(preserve_waiting_since: false)
    @handoff_message = create_outgoing_message(
      @assistant.config['handoff_message'].presence || I18n.t('conversations.tekomi.handoff'),
  ...
  def capture_assistant_session(result_message:, credits_consumed:)
    Tekomi::Assistant::SessionCaptureService.new(assistant: @assistant, conversation: @conversation, run_result: @run_result,
  ...
  def handle_error(error)
    ...
    record_v2_response_failure(error_action_reason(error)) if tekomi_v2_enabled?
  ...
  def process_error_handoff
    return unless conversation_pending?
    return if tekomi_v2_enabled? && newer_customer_message_arrived?

    process_v1_handoff
    record_v2_failure_handoff(source: Tekomi::ConversationEvents::Sources::GENERATION_FAILURE) if tekomi_v2_enabled?
  end
  ...
  def tekomi_v2_enabled?
    account.feature_enabled?('tekomi_integration_v2')
  end

  def report_v1_handoff_not_executed
    error = StandardError.new("Tekomi V1 handoff requested but conversation #{@conversation.display_id} is still pending")
    ChatwootExceptionTracker.new(error, account: account).capture_exception
    Rails.logger.error(
      "[TEKOMI][ResponseBuilderJob] V1 handoff requested but not executed for account=#{account.id} " \
