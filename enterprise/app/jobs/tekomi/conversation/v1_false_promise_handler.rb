module Tekomi::Conversation::V1FalsePromiseHandler
  ...
  def repair_v1_false_promise_response(message_history)
    ...
    Rails.logger.warn(
      "[TEKOMI][ResponseBuilderJob] V1 false promise harness failed for account=#{account.id} " \
  ...
  def detect_v1_false_promise(message_history)
    detection = Tekomi::Llm::AssistantFalsePromiseService.new(
      assistant: @assistant,
  ...
  def v1_false_promise_harness_enabled?
    ActiveModel::Type::Boolean.new.cast(account.tekomi_false_promise_harness_enabled)
  end

  def log_v1_false_promise_detection(detection)
    Rails.logger.info(
      "[TEKOMI][ResponseBuilderJob] V1 false promise harness account=#{account.id} " \
