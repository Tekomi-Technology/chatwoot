class Tekomi::Documents::ResponseBuilderJob < ApplicationJob
  queue_as :low
  ...
  def generate_standard_faqs(document)
    Tekomi::Llm::FaqGeneratorService.new(document: document).generate
  end

  def build_paginated_service(document, options)
    Tekomi::Llm::PaginatedFaqGeneratorService.new(
      document,
  ...
  def create_response(faq, document)
    ...
    Rails.logger.error I18n.t('tekomi.documents.response_creation_error', error: e.message)
  end
end
