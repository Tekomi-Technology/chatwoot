class Tekomi::Documents::ScheduleSyncsJob < ApplicationJob
  queue_as :scheduled_jobs

  DEFAULT_PER_ACCOUNT_BATCH_LIMIT = 50
  DEFAULT_GLOBAL_BATCH_LIMIT = 1000
  SYNC_STALE_TIMEOUT = Tekomi::Document::SYNC_STALE_TIMEOUT
  DAILY_SYNC_JITTER = 4.hours
  WEEKLY_SYNC_JITTER = 1.day
  MONTHLY_SYNC_JITTER = 4.days

  def perform(plan_name = nil)
    @per_account_batch_limit = configured_sync_limit('TEKOMI_DOCUMENT_AUTO_SYNC_PER_ACCOUNT_BATCH_LIMIT', DEFAULT_PER_ACCOUNT_BATCH_LIMIT)
    @global_batch_limit = configured_sync_limit('TEKOMI_DOCUMENT_AUTO_SYNC_GLOBAL_BATCH_LIMIT', DEFAULT_GLOBAL_BATCH_LIMIT)
    @remaining_global_capacity = @global_batch_limit
    @plan_name = plan_name.to_s.downcase.presence
    sync_intervals = Enterprise::Account.tekomi_document_sync_intervals
    stats = { accounts_scanned: 0, accounts_enabled: 0, accounts_scheduled: 0, documents_enqueued: 0 }

    Account.joins(:tekomi_documents).distinct.find_each(batch_size: 100) do |account|
      break if @remaining_global_capacity <= 0

      stats[:accounts_scanned] += 1
      next unless account.feature_enabled?('tekomi_document_auto_sync')
      next unless account_in_selected_plan?(account)

      stats[:accounts_enabled] += 1
      interval = account.tekomi_document_sync_interval(sync_intervals)
  ...
  def process_due_document(document, interval, result)
    ...
    Tekomi::Documents::PerformSyncJob.set(queue: :purgable, wait: sync_execution_delay).perform_later(document)
    @remaining_global_capacity -= 1
    result[:enqueued] += 1
  end

  def due_documents(account, interval)
    syncing = Tekomi::Document.sync_statuses[:syncing]
    synced = Tekomi::Document.sync_statuses[:synced]
    failed = Tekomi::Document.sync_statuses[:failed]
    ...
    documents = account.tekomi_documents.syncable.where(status: :available).where(
  ...
  def log_scheduler_summary(stats)
    ...
    Rails.logger.info("[Tekomi::Documents::ScheduleSyncsJob] #{payload.to_json}")
  end
end
