class Tekomi::Documents::PerformSyncJob < MutexApplicationJob
  queue_as :low
  ...
  discard_on(Tekomi::Documents::SyncService::PermanentSyncError)
  ...
  retry_on(
    Tekomi::Documents::SyncService::TransientSyncError,
  ...
  rescue Tekomi::Documents::SyncService::PermanentSyncError => e
    log_failure_and_raise(document, :permanent_failure, e, start_time)
  rescue Tekomi::Documents::SyncService::TransientSyncError => e
  ...
  def perform_sync(document, start_time)
    ...
    result = Tekomi::Documents::SyncService.new(document.reload).perform
  ...
  def log_sync_outcome(document, **fields)
    ...
    Rails.logger.info("[Tekomi::Documents::PerformSyncJob] #{payload.to_json}")
  end
  ...
  def lock_key(document)
    format(::Redis::Alfred::TEKOMI_DOCUMENT_SYNC_MUTEX, document_id: document.id)
  end
