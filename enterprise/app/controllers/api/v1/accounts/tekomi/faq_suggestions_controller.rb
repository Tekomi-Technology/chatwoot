class Api::V1::Accounts::Tekomi::FaqSuggestionsController < Api::V1::Accounts::BaseController
  before_action :current_account
  before_action -> { check_authorization(Tekomi::FaqSuggestion) }
  ...
  def approve
    ...
    @response = Tekomi::FaqSuggestionApprovalService.new(@suggestion, attributes).perform
  end
  ...
  def set_accessible_suggestions
    @suggestions = Tekomi::FaqSuggestionFinder.new(Current.user, Current.account).perform.includes(:assistant).ordered
  end
