class Api::V1::Accounts::Tekomi::AssistantResponsesController < Api::V1::Accounts::BaseController
  ...
  def drilldown
    ...
    render json: Tekomi::ConversationUsageBuilder.new(@response, drilldown_params, usage_column: :used_faq_ids).build
  end

  def create
    @response = Current.account.tekomi_assistant_responses.new(response_params.except(:assistant_id))
  ...
  def set_response_usage_counts
    ...
    @response_usage_counts = Tekomi::ConversationUsageBuilder.conversation_counts(
  ...
  def apply_filters(base_query)
    ...
    if permitted_params[:document_id].present?
      base_query = base_query.where(
        ...
        documentable_type: 'Tekomi::Document'
      )
    end
  ...
  def set_assistant
    @assistant = Current.account.tekomi_assistants.find_by(id: response_params[:assistant_id])
  end

  def set_responses
    @responses = Current.account.tekomi_assistant_responses.includes(:assistant, :documentable).ordered
  end
