class Api::V1::Accounts::Tekomi::CopilotThreadsController < Api::V1::Accounts::BaseController
  ...
  def build_copilot_response(copilot_message)
    if Current.account.usage_limits[:tekomi][:responses][:current_available].positive?
      enqueue_copilot_response(copilot_message)
    else
      copilot_message.copilot_thread.copilot_messages.create!(
        message_type: :assistant,
        message: { content: I18n.t('tekomi.copilot_limit') }
      )
    end
  end
  ...
  def enqueue_reply_suggestion
    Tekomi::Copilot::ReplySuggestionJob.perform_later(
  ...
  def ensure_message
    return render_could_not_create_error(I18n.t('tekomi.copilot_message_required')) if copilot_thread_params[:message].blank?
  end
  ...
  def assistant
    Current.account.tekomi_assistants.find(copilot_thread_params[:assistant_id])
  end
