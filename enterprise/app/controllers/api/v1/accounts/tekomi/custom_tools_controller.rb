class Api::V1::Accounts::Tekomi::CustomToolsController < Api::V1::Accounts::BaseController
  before_action :ensure_custom_tools_enabled
  before_action -> { check_authorization(Tekomi::CustomTool) }
  ...
  rescue Tekomi::CustomTool::LimitExceededError => e
  ...
  def ensure_custom_tools_enabled
    return if Current.account.feature_enabled?('custom_tools') || Current.account.feature_enabled?('tekomi_integration_v2')
  ...
  def account_custom_tools
    @account_custom_tools ||= Current.account.tekomi_custom_tools
  end

  def execute_test_request(tool)
    http_tool = Tekomi::Tools::HttpTool.new(nil, tool)
