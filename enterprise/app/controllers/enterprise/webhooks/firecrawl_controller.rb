class Enterprise::Webhooks::FirecrawlController < ActionController::API
  ...
  def process_payload
    Tekomi::Tools::FirecrawlParserJob.perform_later(assistant_id: assistant.id, payload: payload) if crawl_page_event?
  ...
  def assistant
    @assistant ||= Tekomi::Assistant.find(permitted_params[:assistant_id])
  end
