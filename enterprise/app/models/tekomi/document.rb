...
# Indexes
#
#  idx_tekomi_documents_on_account_assistant_sync_stats        (account_id,assistant_id,sync_status,last_synced_at)
#  idx_tekomi_documents_on_assistant_id_and_external_link_md5  (assistant_id, md5(external_link)) UNIQUE
#  index_tekomi_documents_on_account_id                        (account_id)
#  index_tekomi_documents_on_account_id_and_sync_status        (account_id,sync_status)
#  index_tekomi_documents_on_assistant_id                      (assistant_id)
#  index_tekomi_documents_on_status                            (status)
#
class Tekomi::Document < ApplicationRecord
  class LimitExceededError < StandardError; end
  SYNC_STALE_TIMEOUT = 2.hours
  self.table_name = 'tekomi_documents'
  ...
  def enqueue_crawl_job
    ...
    Tekomi::Documents::CrawlJob.perform_later(self)
  end

  def enqueue_response_builder_job
    return unless should_enqueue_response_builder?

    Tekomi::Documents::ResponseBuilderJob.perform_later(self)
  end
  ...
  def ensure_within_plan_limit
    limits = account.usage_limits[:tekomi][:documents]
    raise LimitExceededError, I18n.t('tekomi.documents.limit_exceeded') unless limits[:current_available].positive?
  end

  def validate_pdf_format
    return unless pdf_file.attached?

    errors.add(:pdf_file, I18n.t('tekomi.documents.pdf_format_error')) unless pdf_file.blob.content_type == 'application/pdf'
  end
  ...
  def validate_file_attachment
    ...
    errors.add(:pdf_file, I18n.t('tekomi.documents.pdf_size_error'))
  end
