# == Schema Information
#
# Table name: tekomi_custom_tools
#
...
# Indexes
#
#  index_tekomi_custom_tools_on_account_id           (account_id)
#  index_tekomi_custom_tools_on_account_id_and_slug  (account_id,slug) UNIQUE
#
class Tekomi::CustomTool < ApplicationRecord
  ...
  self.table_name = 'tekomi_custom_tools'
  ...
  def ensure_within_limit
    ...
    return if account.tekomi_custom_tools.count < MAX_PER_ACCOUNT

    raise LimitExceededError, I18n.t('tekomi.custom_tool.limit_exceeded', limit: MAX_PER_ACCOUNT)
  end
  ...
  def find_unique_slug(base_slug)
    ...
    raise ActiveRecord::RecordNotUnique, I18n.t('tekomi.custom_tool.slug_generation_failed')
  end
