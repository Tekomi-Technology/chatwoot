module Enterprise::Message
  def self.prepended(base)
    base.class_eval do
      ...
      scope :tekomi_response_triggering, lambda {
  ...
  def tekomi_response_triggering?
    return incoming? && !auto_reply_email? unless persisted?

    self.class.tekomi_response_triggering.exists?(id: id)
  end
  ...
  def mark_pending_conversation_as_open_for_human_response
    ...
    begin
      ...
      create_tekomi_auto_open_activity_message
    ensure
  ...
  def tekomi_pending_conversation?
    ...
    ::TekomiInbox.exists?(inbox_id: conversation.inbox_id)
  end
  ...
  def create_tekomi_auto_open_activity_message
    ::Conversations::ActivityMessageJob.perform_later(
      conversation,
      account_id: conversation.account_id,
      inbox_id: conversation.inbox_id,
      message_type: :activity,
      content: I18n.t('conversations.activity.tekomi.auto_opened_after_agent_reply', locale: conversation.account.locale)
    )
  end
end
