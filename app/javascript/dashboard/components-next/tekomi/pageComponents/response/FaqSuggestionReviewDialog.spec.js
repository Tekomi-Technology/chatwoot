...
vi.mock('dashboard/api/tekomi/faqSuggestions', () => ({
  default: { show },
}));
...
describe('FaqSuggestionReviewDialog', () => {
  ...
  it('keeps a visible error when source conversations fail to load', async () => {
    ...
    expect(wrapper.get('[role="alert"]').text()).toContain(
      'TEKOMI.FAQ_SUGGESTIONS.ERRORS.LOAD_DETAILS'
    );
    expect(wrapper.text()).not.toContain(
      'TEKOMI.FAQ_SUGGESTIONS.DETAILS.NO_SOURCES'
    );
  });
  ...
  it('shows the review actions to users who can open the page', async () => {
    ...
    expect(
      ...
      expect.arrayContaining([
        'TEKOMI.FAQ_SUGGESTIONS.DISMISS',
        'TEKOMI.FAQ_SUGGESTIONS.SAVE',
        'TEKOMI.FAQ_SUGGESTIONS.APPROVE_FAQ',
      ])
    );
  });
  ...
  it('disables review actions while source conversations are loading', async () => {
    ...
    expect(
      ...
      {
        label: 'TEKOMI.FAQ_SUGGESTIONS.DISMISS',
        disabled: 'true',
      },
      {
        label: 'TEKOMI.FAQ_SUGGESTIONS.SAVE',
        disabled: 'true',
      },
      {
        label: 'TEKOMI.FAQ_SUGGESTIONS.APPROVE_FAQ',
        disabled: 'true',
      },
    ]);
