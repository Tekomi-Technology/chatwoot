...
// the tekomi/assistants/:id/summary endpoint). Numbers are emphasised with
...
const fetchSummary = async () => {
  ...
  let message = '';
  try {
    const { data } = await TekomiAssistant.getSummary({
...
// used everywhere else for Tekomi output, instead of a bespoke parser. It
...
<template>
  <div v-if="isLoading || welcomeMarkdown" class="flex flex-col gap-3">
    ...
    <div
      ...
      :aria-label="$t('TEKOMI.OVERVIEW.WELCOME.LOADING')"
    >
