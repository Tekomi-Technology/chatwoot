...
const inboxes = useMapGetter('tekomiInboxes/getRecords');
...
watch(
  ...
  id => {
    ...
    if (id) store.dispatch('tekomiInboxes/get', { assistantId: id });
  },
  { immediate: true }
);
...
<template>
  ...
  >
    <div class="flex items-center gap-2 min-w-0">
      ...
      <span class="truncate">
        {{ $t('TEKOMI.OVERVIEW.INBOX_BANNER.TEXT') }}
      </span>
    </div>
    <div class="flex items-center gap-1 shrink-0">
      <button
        ...
        @click="goToInboxes"
      >
        {{ $t('TEKOMI.OVERVIEW.INBOX_BANNER.ACTION') }}
      </button>
      <button
        type="button"
        class="grid rounded-lg size-7 place-content-center hover:bg-n-amber-4"
        :aria-label="$t('TEKOMI.OVERVIEW.INBOX_BANNER.DISMISS')"
