...
const { dispatch, getDrilldown, getterValues } = vi.hoisted(() => ({
  ...
  getDrilldown: vi.fn(),
  getterValues: {
    'tekomiResponses/getUIFlags': { value: { fetchingList: false } },
    'tekomiResponses/getRecords': { value: [] },
    'tekomiResponses/getMeta': { value: { totalCount: 26, page: 1 } },
  },
}));
...
vi.mock('dashboard/api/tekomi/document', () => ({
...
vi.mock('vue-i18n', () => ({
  useI18n: () => ({
    t: (key, { n } = {}) => {
      if (key === 'TEKOMI.DOCUMENTS.USED_IN_CONVERSATIONS') {
...
describe('DocumentDetails', () => {
  ...
  it('requests another FAQ page when the document has more than 25 FAQs', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith('tekomiResponses/get', {
      page: 1,
      assistantId: 7,
      documentId: 42,
    });
    ...
    expect(dispatch).toHaveBeenLastCalledWith('tekomiResponses/get', {
      page: 2,
      assistantId: 7,
      documentId: 42,
    });
  ...
  it('loads document usage in the third details tab', async () => {
    ...
    expect(wrapper.getComponent(TabBarStub).props('tabs')).toEqual([
      {
        key: 'content',
        label: 'TEKOMI.DOCUMENTS.DETAILS.CONTENT_TAB',
      },
      {
        key: 'faqs',
        label: 'TEKOMI.DOCUMENTS.RELATED_RESPONSES.TITLE',
        count: 26,
      },
      {
        key: 'usage',
        label: 'TEKOMI.DOCUMENTS.DETAILS.USED_IN_CONVERSATIONS',
        count: 8,
      },
    ]);
