...
import DocumentFilter from 'dashboard/components-next/tekomi/assistant/DocumentFilter.vue';
import DocumentBulkActions from 'dashboard/components-next/tekomi/assistant/DocumentBulkActions.vue';
...
import DocumentDetails from 'dashboard/components-next/tekomi/pageComponents/document/DocumentDetails.vue';
import CreateDocumentDialog from 'dashboard/components-next/tekomi/pageComponents/document/CreateDocumentDialog.vue';
import DocumentPageEmptyState from 'dashboard/components-next/tekomi/pageComponents/emptyStates/DocumentPageEmptyState.vue';
import FeatureSpotlightPopover from 'dashboard/components-next/feature-spotlight/FeatureSpotlightPopover.vue';
import LimitBanner from 'dashboard/components-next/tekomi/pageComponents/document/LimitBanner.vue';
import TekomiDocumentAPI from 'dashboard/api/tekomi/document';
...
const uiFlags = useMapGetter('tekomiDocuments/getUIFlags');
const documents = useMapGetter('tekomiDocuments/getRecords');
const isFetching = computed(() => uiFlags.value.fetchingList);
const documentsMeta = useMapGetter('tekomiDocuments/getMeta');
...
const fetchDocuments = async (page = 1, { showLoader = true } = {}) => {
  ...
  if (showLoader) {
    ...
    store.dispatch('tekomiDocuments/setFetchingList', true);
  }

  try {
    const response = await TekomiDocumentAPI.get(filterParams);
    ...
    store.dispatch('tekomiDocuments/setRecords', { records: payload, meta });
    ...
    if (showLoader && fetchingListRequestId === requestId) {
      ...
      store.dispatch('tekomiDocuments/setFetchingList', false);
    }
  }
};
...
const handleSync = async id => {
  try {
    await store.dispatch('tekomiDocuments/sync', id);
    useAlert(t('TEKOMI.DOCUMENTS.SYNC.QUEUED_MESSAGE'));
    scheduleSyncPoll({ extendWindow: true });
  } catch (error) {
    useAlert(t('TEKOMI.DOCUMENTS.SYNC.ERROR_MESSAGE'));
  }
};

const handleAction = ({ action, id }) => {
  selectedDocument.value = documents.value.find(
    tekomiDocument => id === tekomiDocument.id
  );
...
<template>
  <PageLayout
    :header-title="$t('TEKOMI.DOCUMENTS.HEADER')"
    :button-label="$t('TEKOMI.DOCUMENTS.ADD_NEW')"
  ...
  >
    <template #search>
      ...
      >
        <Input
          v-model="searchQuery"
          :placeholder="$t('TEKOMI.DOCUMENTS.FILTERS.SEARCH_PLACEHOLDER')"
    ...
    <template #knowMore>
      <FeatureSpotlightPopover
        ...
        fallback-thumbnail="/assets/images/dashboard/tekomi/document-popover-light.svg"
        fallback-thumbnail-dark="/assets/images/dashboard/tekomi/document-popover-dark.svg"
    ...
    <template #body>
      ...
      >
        <span class="text-base font-medium text-n-slate-12">
          {{ $t('TEKOMI.DOCUMENTS.EMPTY_STATE.FILTERED_TITLE') }}
        </span>
        <span class="max-w-md text-sm text-n-slate-11">
          {{ $t('TEKOMI.DOCUMENTS.EMPTY_STATE.FILTERED_SUBTITLE') }}
        </span>
      </div>
    ...
    <DocumentDetails
      ...
      :tekomi-document="selectedDocument"
