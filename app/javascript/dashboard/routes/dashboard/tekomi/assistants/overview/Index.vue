...
import TekomiAssistant from 'dashboard/api/tekomi/assistant';
...
import RangeSelector from 'dashboard/components-next/tekomi/pageComponents/overview/RangeSelector.vue';
import WelcomeCard from 'dashboard/components-next/tekomi/pageComponents/overview/WelcomeCard.vue';
import MetricCard from 'dashboard/components-next/tekomi/pageComponents/overview/MetricCard.vue';
import AssistantDrilldownDrawer from 'dashboard/components-next/tekomi/pageComponents/overview/AssistantDrilldownDrawer.vue';
import KnowledgeCard from 'dashboard/components-next/tekomi/pageComponents/overview/KnowledgeCard.vue';
import QuickLinks from 'dashboard/components-next/tekomi/pageComponents/overview/QuickLinks.vue';
import InboxBanner from 'dashboard/components-next/tekomi/pageComponents/overview/InboxBanner.vue';
import CoverageBanner from 'dashboard/components-next/tekomi/pageComponents/overview/CoverageBanner.vue';
...
const fetchFaqStats = async () => {
  ...
  try {
    const { data } = await TekomiAssistant.getFaqStats({
      assistantId: assistantId.value,
      signal,
    });
...
const metrics = computed(() => [
  {
    ...
    label: t('TEKOMI.OVERVIEW.METRICS.HANDLED.LABEL'),
    hint: t('TEKOMI.OVERVIEW.METRICS.HANDLED.HINT'),
    ...metricFor('conversations_handled', v => v.toLocaleString(), 'up'),
  },
  {
    key: 'autoResolution',
    metric: 'auto_resolution_rate',
    label: t('TEKOMI.OVERVIEW.METRICS.AUTO_RESOLUTION.LABEL'),
    hint: t('TEKOMI.OVERVIEW.METRICS.AUTO_RESOLUTION.HINT'),
    ...metricFor('auto_resolution_rate', v => `${v}%`, 'up', 'point'),
  },
  {
    key: 'handoff',
    metric: 'handoff_rate',
    label: t('TEKOMI.OVERVIEW.METRICS.HANDOFF.LABEL'),
    hint: t('TEKOMI.OVERVIEW.METRICS.HANDOFF.HINT'),
    ...metricFor('handoff_rate', v => `${v}%`, 'down', 'point'),
  },
  {
    key: 'reopen',
    metric: 'reopen_rate',
    label: t('TEKOMI.OVERVIEW.METRICS.REOPEN.LABEL'),
    hint: t('TEKOMI.OVERVIEW.METRICS.REOPEN.HINT'),
    ...metricFor('reopen_rate', v => `${v}%`, 'down', 'point'),
  },
  {
    key: 'hoursSaved',
    label: t('TEKOMI.OVERVIEW.METRICS.HOURS_SAVED.LABEL'),
    hint: t('TEKOMI.OVERVIEW.METRICS.HOURS_SAVED.HINT'),
    ...metricFor('hours_saved', formatDuration, 'up'),
  },
  {
    key: 'depth',
    label: t('TEKOMI.OVERVIEW.METRICS.DEPTH.LABEL'),
    hint: t('TEKOMI.OVERVIEW.METRICS.DEPTH.HINT'),
    ...metricFor(
      'conversation_depth',
      v => v.toFixed(1),
      'neutral',
...
<template>
  <PageLayout
    :header-title="$t('TEKOMI.OVERVIEW.HEADER')"
    :is-empty="false"
