...
vi.mock('dashboard/composables/store', async () => {
  ...
  mocks.getterValues = {
    'tekomiFaqSuggestions/getRecords': ref([]),
    'tekomiFaqSuggestions/getMeta': ref({ totalCount: 0, page: 1 }),
    'tekomiFaqSuggestions/getUIFlags': ref({
      fetchingList: false,
      updatingItem: false,
      deletingItem: false,
    }),
...
describe('FaqSuggestions', () => {
  beforeEach(() => {
    ...
    mocks.route.query = {};
    mocks.getterValues['tekomiFaqSuggestions/getRecords'].value = [];
    mocks.getterValues['tekomiFaqSuggestions/getMeta'].value = {
      totalCount: 0,
      page: 1,
    };
    mocks.dispatch.mockImplementation((action, payload) => {
      if (action !== 'tekomiFaqSuggestions/setRecords') return;
