...
import FaqSuggestionCard from 'dashboard/components-next/tekomi/assistant/FaqSuggestionCard.vue';
import FaqSuggestionReviewDialog from 'dashboard/components-next/tekomi/pageComponents/response/FaqSuggestionReviewDialog.vue';
...
const suggestions = useMapGetter('tekomiFaqSuggestions/getRecords');
const suggestionMeta = useMapGetter('tekomiFaqSuggestions/getMeta');
...
const fetchSuggestions = async (page = 1) => {
  ...
  store.dispatch('tekomiFaqSuggestions/setFetchingList', true);

  try {
    const response = await runListRequest(signal =>
      TekomiFaqSuggestionsAPI.get({
        page,
    ...
    store.dispatch('tekomiFaqSuggestions/setRecords', {
      records: response.data.payload,
      meta: response.data.meta,
    });
    store.dispatch('tekomiFaqSuggestions/setFetchingList', false);
  } catch (error) {
    useAlert(error?.message || t('TEKOMI.FAQ_SUGGESTIONS.ERRORS.LOAD'));
    store.dispatch('tekomiFaqSuggestions/setFetchingList', false);
  }
};
...
const handleDismiss = async suggestion => {
  activeSuggestionId.value = suggestion.id;
  try {
    await store.dispatch('tekomiFaqSuggestions/dismiss', suggestion.id);
...
<template>
  <PageLayout
    ...
    :header-title="$t('TEKOMI.FAQ_SUGGESTIONS.HEADER')"
  ...
  >
    <template #search>
      <Input
        v-model="searchQuery"
        :placeholder="$t('TEKOMI.FAQ_SUGGESTIONS.SEARCH_PLACEHOLDER')"
        class="w-64"
        size="sm"
        type="search"
        autofocus
    ...
    <template #subHeader>
      ...
      >
        <span class="font-medium text-n-slate-12">
          {{
            $t('TEKOMI.FAQ_SUGGESTIONS.QUEUE_COUNT', {
              count: suggestionMeta.totalCount,
            })
          }}
        </span>
        <span class="border-s border-n-weak ps-2">
          {{ $t('TEKOMI.FAQ_SUGGESTIONS.QUEUE_HINT') }}
        </span>
      </div>
    </template>
    ...
    <template #emptyState>
      <EmptyStateLayout
        :title="$t('TEKOMI.FAQ_SUGGESTIONS.EMPTY_STATE.TITLE')"
        :subtitle="$t('TEKOMI.FAQ_SUGGESTIONS.EMPTY_STATE.SUBTITLE')"
        :show-backdrop="false"
      >
        <template #actions>
          <Button
            v-if="hasActiveFilters"
            :label="$t('TEKOMI.FAQ_SUGGESTIONS.EMPTY_STATE.CLEAR_SEARCH')"
            variant="link"
            size="sm"
            @click="clearFilters"
          />
